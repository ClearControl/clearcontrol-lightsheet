 ...
 */
public class HybridInterleavedOpticsPrefusedSplitImageDataInstruction extends
                                                                      ...
                                                                      LoggingFeature
{
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    final HybridInterleavedOpticsPrefusedImageDataContainer lContainer =
                                                                       lDataWarehouse.getOldestContainer(HybridInterleavedOpticsPrefusedImageDataContainer.class);

    SequentialImageDataContainer lSequentialContainer =
                                                      new SequentialImageDataContainer(getLightSheetMicroscope());
    OpticsPrefusedImageDataContainer lOpticsPrefusedContainer =
                                                              new OpticsPrefusedImageDataContainer(getLightSheetMicroscope());

    for (int d =
               0; d < getLightSheetMicroscope().getNumberOfDetectionArms(); d++)
    {
      String lInputImageKey = "C" + d
                              + "hybrid_interleaved_opticsprefused";

      StackInterface lStack = lContainer.get(lInputImageKey);

      ClearCLIJ clij = ClearCLIJ.getInstance();
      ClearCLImage fullCLImage = clij.converter(lStack)
                                     .getClearCLImage();

      int numberOfLightSheets =
                              getLightSheetMicroscope().getNumberOfLightSheets();

      ClearCLImage[] splitImages =
      ...
      {
        StackInterface lVirtualSequentialStack =
                                               clij.converter(splitImages[l])
                                                   .getStack();

        lVirtualSequentialStack.setMetaData(lStack.getMetaData()
                                                  .clone());
        lVirtualSequentialStack.getMetaData()
                               .removeEntry(MetaDataChannel.Channel);
        lVirtualSequentialStack.getMetaData().addEntry(
        ...
        lVirtualSequentialStack.getMetaData()
                               .addEntry(MetaDataView.LightSheet, l);
        lSequentialContainer.put("C" + d
                                 + "L"
                                 + l,
                                 lVirtualSequentialStack);
      }

      // Fill the virtual opticsprefused container
      StackInterface lOpticsPrefusedStack = clij
                                                .converter(splitImages[splitImages.length
                                                                       - 1])
                                                .getStack();
      lOpticsPrefusedStack.setMetaData(lStack.getMetaData().clone());
      lOpticsPrefusedStack.getMetaData()
                          .removeEntry(MetaDataChannel.Channel);
      lOpticsPrefusedStack.getMetaData().addEntry(
                                                  MetaDataChannel.Channel,
                                                  "opticsprefused");
      lOpticsPrefusedStack.getMetaData()
                          .removeEntry(MetaDataAcquisitionType.AcquisitionType);
      ...
      lOpticsPrefusedContainer.put("C" + d
                                   + "opticsprefused",
                                   lOpticsPrefusedStack);
    ...
    getLightSheetMicroscope().getDataWarehouse()
                             .put("virtual_sequential_" + pTimePoint,
                                  lSequentialContainer);
    getLightSheetMicroscope().getDataWarehouse().put(
                                                     "virtual_opticsprefused_"
                                                     + pTimePoint,
