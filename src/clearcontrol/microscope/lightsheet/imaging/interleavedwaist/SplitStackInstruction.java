...
/**
 * The SplitStackInstruction receives the oldest StackInterfaceContainer from
 * the Warehouse, reads one stack per camera from it and splits it into a given
 * number of sub stacks. These stacke could then be furthere processed, e.g. by
 * Tenengrad fusion
 *
 * Todo: find a way to keep the images in the GPU. Copying back and forth may be
 * wasting time.
 *
 * Author: @haesleinhuepf September 2018
 */
public class SplitStackInstruction extends
                                   ...
                                   PropertyIOableInstructionInterface
{
  ...
  private BoundedVariable<Integer> numberOfStacks =
                                                  new BoundedVariable<Integer>("Number of stacks",
                                                                               9,
                                                                               1,
                                                                               Integer.MAX_VALUE);
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    final StackInterfaceContainer lContainer =
                                             lDataWarehouse.getOldestContainer(StackInterfaceContainer.class);

    SequentialImageDataContainer resultContainer =
               ...
               0; d < getLightSheetMicroscope().getNumberOfDetectionArms(); d++)
    {
      ...
      ClearCLImage fullCLImage = clij.converter(stack)
                                     .getClearCLImage();
      ...
      for (int l = 0; l < numberOfStacks; l++)
      {
        StackInterface resultStack = clij.converter(splitImages[l])
                                         .getStack();

        resultStack.setMetaData(stack.getMetaData().clone());
        resultStack.getMetaData()
                   .removeEntry(MetaDataChannel.Channel);
        resultStack.getMetaData().addEntry(MetaDataChannel.Channel,
                                           "sequential");
        resultStack.getMetaData()
  ...
  {
    SplitStackInstruction copied =
                                 new SplitStackInstruction(getLightSheetMicroscope());
