...
/**
 * AppendConsecutiveSingleViewImagingInstruction appends a list of imaging,
 * fusion and io instructions at the current position in the timelapse
 *
 * Author: @haesleinhuepf 05 2018
 */
public class AppendConsecutiveSingleViewImagingInstruction extends
                                                           ...
                                                           PropertyIOableInstructionInterface
{
  ...
  private final BoundedVariable<Integer> mNumberOfImages =
                                                         new BoundedVariable<Integer>("Number of images",
                                                                                      100);
  private final BoundedVariable<Double> mIntervalInSeconds =
                                                           new BoundedVariable<Double>("Frame delay in s",
                                                                                       15.0);
   ...
   *
   */
  public AppendConsecutiveSingleViewImagingInstruction(int pDetectionArmIndex,
                                                       int pLightSheetIndex,
                                                       int pNumberOfImages,
                                                       double pIntervalInSeconds,
                                                       LightSheetMicroscope pLightSheetMicroscope)
  {
    super("Smart: Append a single view scan with " + pNumberOfImages
          + " images every "
          + pIntervalInSeconds
    ...
    mLightSheetIndex = new BoundedVariable<Integer>("Light sheet",
                                                    pLightSheetIndex,
                                                    0,
                                                    pLightSheetMicroscope.getNumberOfLightSheets()
                                                       - 1);
    mDetectionArmIndex = new BoundedVariable<Integer>("Detection arm",
                                                      pDetectionArmIndex,
                                                      0,
  ...
  {
    String timeMeasurementKey = "sequential_"
    ...
    for (int i = 0; i < mNumberOfImages.get(); i++)
    {
      ...
      index++;
      schedule.add(index,
                   new SingleViewAcquisitionInstruction(mDetectionArmIndex.get(),
                                                        mLightSheetIndex.get(),
                                                        getLightSheetMicroscope()));
      index++;
      schedule.add(index,
                   new WriteSingleLightSheetImageAsRawToDiscInstruction(mDetectionArmIndex.get(),
                                                                        mLightSheetIndex.get(),
                                                                        getLightSheetMicroscope()));
      index++;
      schedule.add(index,
  ...
  {
    return new AppendConsecutiveSingleViewImagingInstruction(mDetectionArmIndex.get(),
                                                             mLightSheetIndex.get(),
                                                             mNumberOfImages.get(),
                                                             mIntervalInSeconds.get(),
                                                             getLightSheetMicroscope());
  }
