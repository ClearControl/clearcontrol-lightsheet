...
/**
 * The AppendConsecutiveInterleavedImagingInstruction appends a list of imaging, fusion and io schedulers at the current position
 * in the timelapse
 *
 * Author: @haesleinhuepf
 * 05 2018
 */
public class AppendConsecutiveInterleavedImagingInstruction extends InstructionBase implements LoggingFeature {
     ...
     *
     */
    public AppendConsecutiveInterleavedImagingInstruction(int pNumberOfImages, double pIntervalInSeconds) {
    ...
    public boolean enqueue(long pTimePoint) {
        ...
        for (int i = 0; i < mNumberOfImages; i ++) {
            ...
            index++;
            schedule.add(index, new InterleavedAcquisitionInstruction());
            index++;
            schedule.add(index, new InterleavedFusionInstruction());
            index++;
            schedule.add(index, new DropOldestStackInterfaceContainerInstruction(InterleavedImageDataContainer.class));
            index++;
            schedule.add(index, new WriteFusedImageAsRawToDiscInstruction("interleaved"));
            index++;
