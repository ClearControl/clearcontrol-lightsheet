                                         ...
                                         LoggingFeature
{
  ...
  public boolean enqueue(long l)
  {
    ...
    // SingleViewAcquisitionInstruction lImage =
    // mLightSheetMicroscope.getDevice(SingleViewAcquisitionInstruction.class,
    // 0);
    ...
    ImagePlus impBeforeShift = clij.converter(acquiredImageStack)
                                   .getImagePlus();
    ...
    ImagePlus impAfterShift = clij.converter(acquiredImageStack)
                                  .getImagePlus();

    // calculate center of mass for both images
    ImageStatistics beforeStats =
                                impBeforeShift.getStatistics(Measurements.CENTER_OF_MASS);
    ImageStatistics afterStats =
                               impAfterShift.getStatistics(Measurements.CENTER_OF_MASS);

    // determine how many pixels the image was shifted
    double virtualShiftX = afterStats.xCenterOfMass
                           - beforeStats.xCenterOfMass;
    double virtualShiftY = afterStats.yCenterOfMass
                           - beforeStats.yCenterOfMass;
    ...
    double realShiftToDoX =
                          -(afterStats.xCenterOfMass - wishPositionX)
                            / virtualShiftX * realShiftX;
    double realShiftToDoY =
                          -(afterStats.yCenterOfMass - wishPositionY)
                            / virtualShiftY * realShiftY;
    ...
    acquiredImageStack = imager.acquire();
    ImagePlus impFinalShift = clij.converter(acquiredImageStack)
                                  .getImagePlus();
    // clij.show(acquiredImageStack, "after final shift");

    ImageStatistics finalStats =
                               impFinalShift.getStatistics(Measurements.CENTER_OF_MASS);
