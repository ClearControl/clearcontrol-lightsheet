...
public class AdaptationStateEnginePanel extends CustomGridPane
{
  ...
  private TitledPane buildLightSheetPanel(LightSheetInterface pLightSheetInterface)
  {
    ...
    lCustomGridPane.add(buildVariableLabel("",
  ...
  private TitledPane buildModuleStatePanel(HasControlPlaneState pHasControlPlaneState)
  {
    ...
    int lNumberOfControlPlanes =
                               mInterpolatedAcquisitionState.getNumberOfControlPlanes();

    ControlPlaneStatePanel lControlPlaneStatePanel =
                                                   new ControlPlaneStatePanel(pHasControlPlaneState,
    ...
    {
      lName =
            ((ReadOnlyNameableInterface) pHasControlPlaneState).getName();
    }

    TitledPane lTitledPane = new TitledPane("Adaptation " + lName
                                            + " state",
                                            lControlPlaneStatePanel);
    lTitledPane.setAnimated(false);
    lTitledPane.setExpanded(true);

    if (pHasControlPlaneState instanceof CanBeActive
        && !((CanBeActive) pHasControlPlaneState).isActive())
    {
