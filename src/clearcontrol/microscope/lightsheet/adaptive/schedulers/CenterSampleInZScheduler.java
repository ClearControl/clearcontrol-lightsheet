...
public class CenterSampleInZScheduler  extends
    ...
    BoundedVariable<Double> mSearchRangeMinZVariable = new BoundedVariable<Double>("search range start", 10.0, -Double.MAX_VALUE, Double.MAX_VALUE, 0.001);
    BoundedVariable<Double> mSearchRangeMaxZVariable = new BoundedVariable<Double>("search range end", 12.0, -Double.MAX_VALUE, Double.MAX_VALUE, 0.001);

    BoundedVariable<Double> mSearchRangeZVariable = new BoundedVariable<Double>("search range", 2.0, -Double.MAX_VALUE, Double.MAX_VALUE, 0.001);
    ...
    public boolean enqueue(long l) {
        ...
        double stepSize = (mSearchRangeMaxZVariable.get() - mSearchRangeMinZVariable.get()) / mNumberOfStepsVariable.get();
        double maxValue = 0;
        ...
        for (int i = 0; i < mNumberOfStepsVariable.get(); i++) {
            ...
            ImagePlus imp = clij.converter(acquiredImageStack).getImagePlus();
        ...
        lStageZ.moveBy(bestPositionZ - lStageZ.getPositionVariable().get(), true);
