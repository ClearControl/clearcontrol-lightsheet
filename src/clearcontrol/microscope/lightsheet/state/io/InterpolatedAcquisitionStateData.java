...
public class InterpolatedAcquisitionStateData
{
  ...
  /**
   * This constructor is needed for file I/O it should not be used by
   * the programmer
   */
  @Deprecated
  public InterpolatedAcquisitionStateData(){}
  ...
  public InterpolatedAcquisitionStateData(InterpolatedAcquisitionState pState) {
    ...
    for (int d = 0; d < pState.getNumberOfDetectionArms(); d++) {
      ...
      for (int cpi = 0; cpi < pState.getNumberOfControlPlanes(); cpi++) {
        mDetectionZ[d][cpi] = pState.get(
            LightSheetDOF.DZ, cpi, d);
      }
    }
    ...
    for (int l = 0; l < pState.getNumberOfLightSheets(); l++)
    {
      ...
      for (int cpi = 0; cpi < pState.getNumberOfControlPlanes(); cpi++)
      {
        mIlluminationX[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IX, cpi, l);
        mIlluminationY[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IY, cpi, l);
        mIlluminationZ[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IZ, cpi, l);
        mIlluminationA[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IA, cpi, l);
        mIlluminationB[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IB, cpi, l);
        mIlluminationW[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IW, cpi, l);
        mIlluminationH[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IH, cpi, l);
        mIlluminationP[l][cpi] = pState.getInterpolationTables().get(LightSheetDOF.IP, cpi, l);
      }
    }
  }
  ...
  public void copyTo(InterpolatedAcquisitionState pState) {
    for (int d = 0; d < pState.getNumberOfDetectionArms(); d++) {
      for (int cpi = 0; cpi < pState.getNumberOfControlPlanes(); cpi++) {
        pState.getInterpolationTables().set(LightSheetDOF.DZ, cpi, d, mDetectionZ[d][cpi]);
      }
    }
    ...
    for (int l = 0; l < pState.getNumberOfLightSheets(); l++)
    {
      for (int cpi = 0; cpi < pState.getNumberOfControlPlanes(); cpi++)
      {
        pState.getInterpolationTables().set(LightSheetDOF.IX, cpi, l, mIlluminationX[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IY, cpi, l, mIlluminationY[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IZ, cpi, l, mIlluminationZ[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IA, cpi, l, mIlluminationA[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IB, cpi, l, mIlluminationB[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IW, cpi, l, mIlluminationW[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IH, cpi, l, mIlluminationH[l][cpi]);
        pState.getInterpolationTables().set(LightSheetDOF.IP, cpi, l, mIlluminationP[l][cpi]);
      }
    }
  }
}
