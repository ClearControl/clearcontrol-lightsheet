...
public class DenseMatrixIODemo
{
  ...
  public void testMatrixIO() throws IOException
  {
    ...
    // save matrix
    DenseMatrix64FWriter lWriter = new DenseMatrix64FWriter(lTempFile,
                                                            lMatrix);
    assertTrue(lWriter.write());

    // load matrix
    DenseMatrix64FReader lReader =
                                 new DenseMatrix64FReader(lTempFile,
                                                          lMatrixReloaded);
    ...
    DenseMatrix64FReader lReader2 =
                                  new DenseMatrix64FReader(lTempFile);
    lReader2.read();
    DenseMatrix64F lMatrixRevolutions = lReader2.getMatrix();

    // check if content is the same
    assertTrue(Arrays.equals(lMatrixRevolutions.data,
                             lMatrixReloaded.data));
    System.out.println("O: "
                       + Arrays.toString(lMatrixRevolutions.data));
  }
}
