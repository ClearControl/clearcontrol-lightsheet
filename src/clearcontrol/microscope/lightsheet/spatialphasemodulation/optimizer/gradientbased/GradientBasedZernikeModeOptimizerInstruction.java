...
/**
 * The GradientBasedZernikeModeOptimizerInstruction allows optimizing the
 * deformable mirror. It optimizes a single Zernike factor in the array taken
 * from the mirror device. Was successfully tested on defocus (Z4). It is
 * recommend to run it only on images where there is enough content (single
 * beads are on enough)
 *
 * Author: @haesleinhuepf 05 2018
 */
public class GradientBasedZernikeModeOptimizerInstruction extends
                                                          ...
                                                          PropertyIOableInstructionInterface
{
  ...
  private BoundedVariable<Double> stepSize =
  ...
  public GradientBasedZernikeModeOptimizerInstruction(LightSheetMicroscope pLightSheetMicroscope,
                                                      SpatialPhaseModulatorDeviceInterface pSpatialPhaseModulatorDeviceInterface,
                                                      int pZernikeFactorIndexToOptimize)
  {
    super("Adaptive optics: Gradient based Z"
          + ZernikePolynomials.jNoll(pZernikeFactorIndexToOptimize)
          + "("
          + ZernikePolynomials.getZernikeModeName(pZernikeFactorIndexToOptimize)
          + ")"
          + " optimizer for "
  ...
  public boolean initialize()
  {
    ...
    mPositionZ =
               new BoundedVariable<Double>("position Z",
                                           (lState.getStackZLowVariable()
                                                  .get()
                                                  .doubleValue()
                                            + lState.getStackZHighVariable()
                                                    .get()
                                                    .doubleValue())
                                                         / 2,
                                           lState.getStackZLowVariable()
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    System.arraycopy(zernikes,
                     ...
                     0,
                     zernikes.length);
    zernikesFactorDecreased[mZernikeFactorIndexToOptimize] -=
                                                           stepSize.get();
    ZernikeSolution zernikeSolutionFactorDecrement =
                                                   new ZernikeSolution(zernikesFactorDecreased,
    ...
    System.arraycopy(zernikes,
                     ...
                     0,
                     zernikes.length);
    zernikesFactorIncreased[mZernikeFactorIndexToOptimize] +=
                                                           stepSize.get();
    ZernikeSolution zernikeSolutionFactorIncrement =
                                                   new ZernikeSolution(zernikesFactorIncreased,
                                                                       getLightSheetMicroscope(),
                                                                       mSpatialPhaseModulatorDeviceInterface,
                                                                       mPositionZ.get());

    // determine fitness of both solutions
    double factorDecrementQuality =
                                  zernikeSolutionFactorDecrement.fitness();
    double factorIncrementQuality =
                                  zernikeSolutionFactorIncrement.fitness();
  ...
  {
    return new GradientBasedZernikeModeOptimizerInstruction(getLightSheetMicroscope(),
                                                            mSpatialPhaseModulatorDeviceInterface,
                                                            mZernikeFactorIndexToOptimize);
  }
