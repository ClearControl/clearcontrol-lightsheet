 ...
 */
public class SensorLessAOForSinglePlaneInstruction extends
                                                   ...
                                                   PropertyIOableInstructionInterface
{
  ...
  private BoundedVariable<Double> mPositionZ =
                                             new BoundedVariable<Double>("position Z",
                                                                         50.0,
                                                                         0.0,
                                                                         100.0);
  private BoundedVariable<Double> mStepSize =
                                            new BoundedVariable<Double>("Aberration step size",
                                                                        0.02,
                                                                        0.0,
                                                                        2.0,
                                                                        0.0000000001);
  private BoundedVariable<Integer> mZernikeFactor =
                                                  new BoundedVariable<Integer>("Zernike Factor",
                                                                               3,
                                                                               0,
                                                                               66);
  private BoundedVariable<Integer> mNumberOfTilesX =
                                                   new BoundedVariable<Integer>("Number Of Tiles On X",
                                                                                1,
                                                                                0,
                                                                                2048);
  private BoundedVariable<Integer> mNumberOfTilesY =
                                                   new BoundedVariable<Integer>("Number Of Tiles On Y",
                                                                                1,
                                                                                0,
                                                                                2048);;
  ...
  WriteSingleLightSheetImageAsTifToDiscInstruction lWrite =
                                                          new WriteSingleLightSheetImageAsTifToDiscInstruction(0,
                                                                                                               0,
                                               ...
                                               SpatialPhaseModulatorDeviceInterface pSpatialPhaseModulatorDeviceInterface)
  {
    ...
    this.mSpatialPhaseModulatorDeviceInterface =
  ...
  {
    zernikes =
             mSpatialPhaseModulatorDeviceInterface.getZernikeFactors();
  ...
  public boolean optimize() throws InterruptedException
  {
    ...
    mTileHeight = (int) lDefaultStack.getHeight()
                  / mNumberOfTilesY.get();
    mTileWidth =
               (int) lDefaultStack.getWidth() / mNumberOfTilesX.get();

    double[][] lDefaultQuality =
                               determineTileWiseQuality(lDefaultStack);
    ...
    for (int i = 0; i < mNumberOfTilesX.get() * mNumberOfTilesY.get()
                        - 1; i++)
    {
      if (getLightSheetMicroscope().getTimelapse()
                                   .getStopSignalVariable()
                                   .get())
      {
        return false;
      }
    ...
    System.arraycopy(zernikes,
                     0,
                     zernikesFactorDecreased,
                     0,
                     zernikes.length);
    ...
    double[][] lFactorDecreasedQuality =
                                       determineTileWiseQuality(lFactorDecreasedStack);
    ...
    double[][] lFactorIncreasedQuality =
                                       determineTileWiseQuality(lFactorIncreasedStack);
    lFactorIncreasedStack.release();

    double decreasedValue =
                          zernikesFactorDecreased[mZernikeFactor.get()];
    double increasedValue =
                          zernikesFactorIncreased[mZernikeFactor.get()];
    double defaultValue = 0;

    // Tile wise maximum finding
    double[][] lMaxima =
                       new double[mNumberOfTilesX.get()][mNumberOfTilesY.get()];
    for (int x = 0; x < mNumberOfTilesX.get(); x++)
    {
      for (int y = 0; y < mNumberOfTilesY.get(); y++)
      {
        double[] result =
                        CalcParabolaVertex(decreasedValue,
                                           lFactorDecreasedQuality[x][y],
                                           defaultValue,
                                           lDefaultQuality[x][y],
                                           increasedValue,
                                           lFactorIncreasedQuality[x][y]);
        if (result[0] > 10 || result[0] < -10)
        {
          info("Optimizer trying to set extreme amount of optimization"
               + result[0]);
          result[0] = 0.0;
        }
    ...
    System.out.println("Zernikes factor decreased state"
                       + Arrays.toString(zernikesFactorDecreased));
    System.out.println("Zernikes factor increased state"
                       + Arrays.toString(zernikesFactorIncreased));

    System.out.println("Zernikes default quality: "
                       + Arrays.deepToString(lDefaultQuality));
    System.out.println("Zernikes factor decreased quality: "
                       + Arrays.deepToString(lFactorDecreasedQuality));
    System.out.println("Zernikes factor increased quality: "
                       + Arrays.deepToString(lFactorIncreasedQuality));

    System.out.println("Zernikes for maxima image quality: "
                       + Arrays.deepToString(lMaxima));

    // Taking a stack of images with different mirror modes, crop it and save to
    // disc
  ...
  public void acquireTiledImages(double[][] pMaxima) throws InterruptedException
  {
    ...
    DropAllContainersOfTypeInstruction lRemoveOldContainers =
                                                            new DropAllContainersOfTypeInstruction(StackInterfaceContainer.class,
                                                                                                   getLightSheetMicroscope().getDataWarehouse());
    File lFolder = getLightSheetMicroscope()
    ...
    try
    {
      BufferedWriter lOutputStream =
                                   new BufferedWriter(new FileWriter(lFile));
      ...
      for (int x = 0; x < mNumberOfTilesX.get(); x++)
      {
        ...
        for (int y = 0; y < mNumberOfTilesY.get(); y++)
        {
          ...
          // CropInstruction lCrop = new
          // CropInstruction(getLightSheetMicroscope().getDataWarehouse(),
          // x *mTileWidth, y * mTileHeight ,mTileWidth, mTileHeight);
          // lCrop.enqueue(0);
          lWrite.enqueue(lCounter);
          lRemoveOldContainers.enqueue(lCounter);
          lOutputStream.write(lCounter + "\t"
                              + x * mTileWidth
                              + "\t"
                              + y * mTileHeight
                              + "\t"
  ...
  {
    DiscreteConsinusTransformEntropyPerSliceEstimator lQualityEstimator =
  ...
  public double[][] determineTileWiseQuality(StackInterface lStack)
  {
    ...
    for (int x = 0; x < mNumberOfTilesX.get(); x++)
    {
      for (int y = 0; y < mNumberOfTilesY.get(); y++)
      {
        final StackInterface lTile = crop(lStack,
                                          x * mTileWidth,
                                          y * mTileHeight,
                                          mTileHeight,
                                          mTileWidth);
  ...
  public StackInterface crop(StackInterface lStack,
                             int lCropX,
                             int lCropY,
                             int lHieght,
                             int lWidth)
  {
    ...
    { lWidth, lHieght, lStack.getDepth() }, src.getChannelDataType());
  ...
  // Checked
  public static double[] CalcParabolaVertex(double x1,
                                            double y1,
                                            double x2,
                                            double y2,
                                            double x3,
                                            double y3)
  {
    ...
    double A = (x3 * (y2 - y1) + x2 * (y1 - y3) + x1 * (y3 - y2))
               / denom;
    double B = (x3 * x3 * (y1 - y2) + x2 * x2 * (y3 - y1)
                + x1 * x1 * (y2 - y3))
               / denom;
  ...
  {
    return new clearcontrol.microscope.lightsheet.spatialphasemodulation.optimizer.sensorlessAO.SensorLessAOForSinglePlaneInstruction(getLightSheetMicroscope(),
