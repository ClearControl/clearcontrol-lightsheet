...
/**
 * The MirrorModeImageQualityDeterminer sends a given mirror mode the a mirror
 * device and takes an unfused image afterwards. From this image DCTS2D is
 * calculated as metric for image quality. This may only work if the
 * misalignment of the mirror is not too dramatic and the image has enough
 * content.
 *
 * Author: @haesleinhuepf 04 2018
 */
public class MirrorModeImageQualityDeterminer implements
                                              LoggingFeature
{
  ...
  public MirrorModeImageQualityDeterminer(LightSheetMicroscope pLightSheetMicroscope,
  ...
  {
    DataContainerInterface lContainer =
                                      mLightSheetMicroscope.getDataWarehouse()
                                                           .get(mSpatialPhaseModulatorDeviceInterface.getName()
                                                                + "_"
                                                                + key);
    ...
    else
    {
      ...
      warning("No '" + key
              + "' mirror matix found! Reading from disc");
      File lMirrorModeDirectory =
                                MachineConfiguration.get()
                                                    .getFolder("MirrorModes");

      DenseMatrix64F lFlatMirrorMatrix =
                                       mSpatialPhaseModulatorDeviceInterface.getMatrixReference()
                                                                            .get()
                                                                            .copy();

      new DenseMatrix64FReader(new File(lMirrorModeDirectory,
                                        mSpatialPhaseModulatorDeviceInterface.getName()
                                                              + "_flat.json"),
                               lFlatMirrorMatrix).read();

      MirrorModeContainer lNewContainer =
                                        new MirrorModeContainer(mLightSheetMicroscope.getTimelapse()
                                                                                     .getTimePointCounterVariable()
                                                                                     .get());
      lNewContainer.setMirrorMode(lFlatMirrorMatrix);
      mLightSheetMicroscope.getDataWarehouse()
                           .put(mSpatialPhaseModulatorDeviceInterface.getName()
                                + "_" + key, lNewContainer);
      return (MirrorModeContainer) lContainer;
    }
  }
  ...
  private void determineQuality()
  {
    if (mMatrix != null)
    {
      ...
      DataContainerInterface lActuatorInfluenceMatrixContainer =
                                                               getMirrorModeContainer("actuator_influence");
      if (lActuatorInfluenceMatrixContainer != null)
      {
        DenseMatrix64F lActuatorInfluenceMatrix =
                                                ((MirrorModeContainer) lActuatorInfluenceMatrixContainer).getMirrorMode();
        lMatrixToTest =
                      TransformMatrices.multiplyElementWise(lMatrixToTest,
                                                            lActuatorInfluenceMatrix);
      }
      else
      {
      ...
      DataContainerInterface lFlatMirrorModeContainer =
                                                      getMirrorModeContainer("flat");
      if (lFlatMirrorModeContainer != null)
      {
        DenseMatrix64F lFlatMirrorMatrix =
                                         ((MirrorModeContainer) lFlatMirrorModeContainer).getMirrorMode();
        lMatrixToTest = TransformMatrices.sum(lMatrixToTest,
                                              lFlatMirrorMatrix);
      }
      else
      {
      ...
      double maxZ = currentState.getStackZHighVariable()
                                .get()
                                .doubleValue();
      double stepZ = currentState.getStackZStepVariable()
                                 .get()
                                 .doubleValue();

      SingleViewPlaneImager lImager =
                                    new SingleViewPlaneImager(mLightSheetMicroscope,
                                                              mPositionZ);
    ...
    else
    {
      ...
      DiscreteConsinusTransformEntropyPerSliceEstimator lQualityEstimator =
                                                                          new DiscreteConsinusTransformEntropyPerSliceEstimator(lStack);
      mQuality =
               new Mean().evaluate(lQualityEstimator.getQualityArray());

      // restoreState();
      // currentState = (InterpolatedAcquisitionState)
