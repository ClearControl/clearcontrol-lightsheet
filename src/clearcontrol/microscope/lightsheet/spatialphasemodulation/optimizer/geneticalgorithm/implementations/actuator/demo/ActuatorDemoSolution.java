 ...
 *
 * In this class we override the fitnesses function to make it compare to a
 * reference solution. In that way, the genetic algorithm can be tested without
 * a real microscope.
 *
...
public class ActuatorDemoSolution extends ActuatorSolution
{
  ...
  public ActuatorDemoSolution(DenseMatrix64F pMatrix,
  ...
  public double fitness()
  {
    ...
    for (int x = 0; x < lReferenceMatrix.numCols; x++)
    {
      ...
      {
        mse +=
            Math.pow(lReferenceMatrix.get(y, x) - lMatrix.get(y, x),
                     2);
      }
    }
    return -mse;
