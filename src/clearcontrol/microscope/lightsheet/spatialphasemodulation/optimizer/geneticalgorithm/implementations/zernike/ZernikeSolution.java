...
/**
 * The ZernikeSolution represents a combination of Zernike modes representing a
 * mirror shape. It's fitness function uses a LightSheetMicroscope and a
 * deformable mirror to determine image quality.
 *
...
public class ZernikeSolution implements SolutionInterface
{
  ...
  public ZernikeSolution(double[] pFactors,
                         ...
                         double pPositionZ)
  {
    ...
    initializeZernikePolynomialList(pSpatialPhaseModulatorDeviceInterface.getMatrixWidth(),
  ...
  /**
   * Depreacted because we want to optimize Zernike factors and no matrices
   * anymore
   * 
   * @return
   */
  ...
  public DenseMatrix64F getMatrix()
  {
    ...
    DenseMatrix64F lMatrix = new DenseMatrix64F(lTemplate.numRows,
                                                lTemplate.numCols);

    for (int i = 0; i < mFactors.length; i++)
    {
      DenseMatrix64F lSummand =
                              TransformMatrices.multiply(mZernikePolynomialList.get(i),
                                                         mFactors[i]);
                           ...
                           double similarityTolerance)
  {
    ...
    for (int i = 0; i < mFactors.length; i++)
    {
      if (Math.abs(mFactors[i]
                   - ((ZernikeSolution) s).mFactors[i]) > similarityTolerance)
      {
        return false;
      }
    }
   ...
   * @param pHeight
   */
  private void initializeZernikePolynomialList(int pWidth,
                                               int pHeight)
  {
    ...
    for (int n = 0; n <= sMaxN; n++)
    {
      for (int m = -n; m <= n; m += 2)
      {
        ...
        mZernikePolynomialList.add(new ZernikePolynomialsDenseMatrix64F(pWidth,
                                                                        pHeight,
                                                                        m,
                                                                        n));
      }
