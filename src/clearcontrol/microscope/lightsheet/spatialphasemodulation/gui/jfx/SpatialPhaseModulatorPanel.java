...
public class SpatialPhaseModulatorPanel extends CustomGridPane
{
  ...
  private ArrayList<BoundedVariable<Double>> zernikeFactorVariables =
                                                                    new ArrayList<BoundedVariable<Double>>();
  ...
  public SpatialPhaseModulatorPanel(SpatialPhaseModulatorDeviceInterface pSpatialPhaseModulatorDeviceInterface)
  {
    ...
    zernikeFactors =
                   pSpatialPhaseModulatorDeviceInterface.getZernikeFactors();
    ...
    GridPane.setHalignment(currentZernikeCompositionViewer,
                           HPos.CENTER);
    ...
    GridPane.setHalignment(currentZernikeCompositionNormalizedViewer,
                           HPos.CENTER);
    ...
    int maximumM =
                 (int) Math.sqrt(pSpatialPhaseModulatorDeviceInterface.getZernikeFactors().length);
    ...
    for (int n = 0; n <= maximumM; n++)
    {
      ...
      {
        {
          DenseMatrix64F lZernikeMatrix =
                                        TransformMatrices.multiply(new ZernikePolynomialsDenseMatrix64F(50,
                                                                                                        50,
                                                                                                        m,
                                                                                                        n),
                                                                   1);
          DenseMatrixImage lMatrixImage =
                                        new DenseMatrixImage(lZernikeMatrix,
                                                             new BlueCyanGreenYellowOrangeRedLUT());

          // View icon
          ImagePane matrixViewPane = new ImagePane(50,
                                                   50,
                                                   lMatrixImage);
          ...
          BoundedVariable<Double> zernikeFactorVariable =
                                                        new BoundedVariable<>("Z"
                                                                              + ZernikePolynomials.jNoll(n,
                                                                                                         m)
                                                                              + " (n="
                                                                              + n
                                                                              + ", m="
                                                                              + m
                                                                              + ") "
                                                                              + ZernikePolynomials.getZernikeModeName(n,
                                                                                                                      m),
                                                                              zernikeFactors[index],
                                                                              -Double.MAX_VALUE,
                                                                              Double.MAX_VALUE,
                                                                              0.00000000001);
          NumberVariableTextField<Double> variableTextField =
                                                            new NumberVariableTextField<Double>(zernikeFactorVariable.getName(),
                                                                                                zernikeFactorVariable);
          GridPane.setHalignment(variableTextField.getLabel(),
                                 HPos.CENTER);
          GridPane.setHalignment(variableTextField.getTextField(),
                                 HPos.CENTER);
          add(variableTextField.getLabel(),
              maximumM + m,
              n * 3 + 1,
              2,
              1);
          add(variableTextField.getTextField(),
              maximumM + m,
              n * 3 + 2,
              2,
              1);
          ...
          final int finalM = m;
          final int finalN = n;
          zernikeFactorVariable.addSetListener((oldValue,
                                                newValue) -> {
            if (mInitializing)
            {
              return;
            }
            zernikeFactors[ZernikePolynomials.jNoll(finalN, finalM)
                           - 1] = newValue;
            refreshUI();
          });
  ...
  private void refreshUI()
  {
    ...
    {
      zernikeFactorVariables.get(i)
                            .set(zernikeFactors[ZernikePolynomials.jNoll(i)
                                                - 1]);
    }
    mInitializing = false;

    DenseMatrix64F zernikeCompositionAsMatrix =
                                              ZernikePolynomials.zernikeComposition(zernikeFactors);

    BlueCyanGreenYellowOrangeRedLUT lLookUpTable =
                                                 new BlueCyanGreenYellowOrangeRedLUT();

    DenseMatrixImage lMatrixImage =
                                  new DenseMatrixImage(zernikeCompositionAsMatrix,
                                                       lLookUpTable);

    currentZernikeCompositionViewer.setImage(lMatrixImage);

    double lMin =
                TransformMatrices.getMinOfMatrix(zernikeCompositionAsMatrix);
    double lMax =
                TransformMatrices.getMaxOfMatrix(zernikeCompositionAsMatrix);

    double lFactor = 1.0 / Math.max(Math.abs(lMin), Math.abs(lMax));
    DenseMatrix64F lScaledMatrix =
                                 TransformMatrices.multiply(zernikeCompositionAsMatrix,
                                                            lFactor);
    currentZernikeCompositionNormalizedViewer.setImage(new DenseMatrixImage(lScaledMatrix,
                                                                            lLookUpTable));
