...
/**
 * ZernikePolynomials is a class full of utilities for working with Zernike
 * Modes
 *
 *
 * @author royer
...
public class ZernikePolynomials
{
  ...
  private static void initializeJNollSortedZernikeModes()
  {
    ...
    for (int n = 0; n <= maximumM; n++)
    {
      for (int m = -n; m <= n; m += 2)
      {
        DenseMatrix64F zernikeMode =
                                   new ZernikePolynomialsDenseMatrix64F(MATRIX_SIZE,
                                                                        MATRIX_SIZE,
                                                                        m,
                                                                        n);
  ...
  public static DenseMatrix64F zernikeComposition(double[] factorsSortedByJNoll)
  {
    ...
    for (int i = 0; i < factorsSortedByJNoll.length; i++)
    {
      ...
      {
        DenseMatrix64F weightedMatrix =
                                      TransformMatrices.multiply(jNollSortedZernikeModes[i],
                                                                 factorsSortedByJNoll[i]);
