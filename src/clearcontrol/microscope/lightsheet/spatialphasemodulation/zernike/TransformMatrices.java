...
public class TransformMatrices
{
   ...
   * @return M*s
   */
  public static DenseMatrix64F multiply(DenseMatrix64F pMatrix,
                                        double pFactor)
  {
    DenseMatrix64F lResultMatrix =
                                 new DenseMatrix64F(pMatrix.numCols,
                                                    pMatrix.numRows);
   ...
   * @return M.*N
   */
  public static DenseMatrix64F multiplyElementWise(DenseMatrix64F pMatrix1,
                                                   DenseMatrix64F pMatrix2)
  {
    DenseMatrix64F lResultMatrix =
                                 new DenseMatrix64F(pMatrix1.numCols,
                                                    pMatrix1.numRows);
    for (int y = 0; y < lResultMatrix.numRows; y++)
    {
      for (int x = 0; x < lResultMatrix.numCols; x++)
      {
        lResultMatrix.set(y,
                          x,
                          pMatrix1.get(y, x) * pMatrix2.get(y, x));
      }
    }
    return lResultMatrix;
  }
  ...
  public static DenseMatrix64F sum(List<DenseMatrix64F> pMatrixList)
  {
    ...
    DenseMatrix64F lSumMatrix =
                              new DenseMatrix64F(lReferenceMatrix.numRows,
                                                 lReferenceMatrix.numCols);
   ...
   * 
   * @param pMatrix1
   *          Matrix M containing elements m_ij
   * @param pMatrix2
   *          Matrix N containing elements n_ij
   * @return MSE = sum_ij(pow(m_ij - n_ij, 2))
   */
  public static double meanSquaredError(DenseMatrix64F pMatrix1,
                                        DenseMatrix64F pMatrix2)
  {
   ...
   * @param pMatrix2
   *          N with elements n_ij
   * @param pTolerance
   *          t
   * @return true if abs(m_ij - n_ij) <= t for all matrix elements
   */
  public static boolean matricesEqual(DenseMatrix64F pMatrix1,
                                      DenseMatrix64F pMatrix2,
                                      double pTolerance)
  {
    if (pMatrix1.numRows != pMatrix2.numRows)
    ...
    for (int y = 0; y < pMatrix1.numRows; y++)
    {
      for (int x = 0; x < pMatrix1.numCols; x++)
      {
        if (Math.abs(pMatrix1.get(y, x)
                     - pMatrix2.get(y, x)) > pTolerance)
        {
          return false;
        }
      }
  ...
  public static boolean flipSquareMatrixVertical(DenseMatrix64F pSourceMatrix,
                                                 DenseMatrix64F pTargetMatrix)
  {
    ...
    for (int y = 0; y < pSourceMatrix.numRows; y++)
    {
      ...
      {
        pTargetMatrix.set(pSourceMatrix.numRows - y
                          - 1,
                          x,
                          pSourceMatrix.get(y, x));
      }
    }
  ...
  public static boolean flipSquareMatrixHorizontal(DenseMatrix64F pSourceMatrix,
                                                   DenseMatrix64F pTargetMatrix)
  {
    if (pSourceMatrix == pTargetMatrix
        || pSourceMatrix.numRows != pTargetMatrix.numRows
  ...
  public static boolean flipSquareMatrixXY(DenseMatrix64F pSourceMatrix,
                                           DenseMatrix64F pTargetMatrix)
  {
    if (pSourceMatrix == pTargetMatrix
        || pSourceMatrix.numRows != pTargetMatrix.numRows
  ...
  {
    TransformMatrices.flipSquareMatrixXY(pSourceMatrix,
                                         pTargetMatrox);
    pSourceMatrix = pTargetMatrox.copy();
    TransformMatrices.flipSquareMatrixVertical(pSourceMatrix,
                                               pTargetMatrox);
  }
                                            ...
                                            DenseMatrix64F pTargetMatrox)
  {
    TransformMatrices.flipSquareMatrixXY(pSourceMatrix,
                                         pTargetMatrox);
    pSourceMatrix = pTargetMatrox.copy();
    TransformMatrices.flipSquareMatrixHorizontal(pSourceMatrix,
                                                 pTargetMatrox);
  }
   ...
   * @return M*N
   */
  public static DenseMatrix64F multiplyMatrix(DenseMatrix64F pMatrix1,
                                              DenseMatrix64F pMatrix2)
  {
    ...
    DenseMatrix64F lResultMatrix =
                                 new DenseMatrix64F(pMatrix1.numRows,
                                                    pMatrix2.numCols);
    for (int y = 0; y < lResultMatrix.numRows; y++)
    {
      for (int x = 0; x < lResultMatrix.numCols; x++)
  ...
  {
    DenseMatrix64F lResultMatrix =
                                 new DenseMatrix64F(pArray.length, 1);
  ...
  {
    DenseMatrix64F lResultMatrix =
                                 new DenseMatrix64F(1, pArray.length);
    for (int y = 0; y < pArray.length; y++)
    {
