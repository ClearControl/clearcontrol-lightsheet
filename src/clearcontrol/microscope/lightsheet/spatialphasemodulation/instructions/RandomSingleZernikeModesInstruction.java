...
public class RandomSingleZernikeModesInstruction extends
                                                 ...
                                                 LoggingFeature
{
  ...
  {
    super("Adaptive optics: Send randomly chosen single random Zernike modes to "
  ...
  {
    double[] lArray =
                    new double[mZernikeModeFactorBasedSpatialPhaseModulatorBase.getZernikeFactors().length];
    double thousands = Math.pow(10, mDigitsAfterDecimal.get());
    List<Integer> lSelectedZernikeModesList =
    ...
    {
      lSelectedChangingMode =
                            lSelectedZernikeModesList.get(mRandom.nextInt(lSelectedZernikeModesList.size()));
    }
    lArray[lSelectedChangingMode] = Math.round(
                                               (mRandom.nextDouble()
                                                * 2.0 - 1.0)
                                               * mRangeOfZernikeCoefficientsArray[lSelectedChangingMode].get()
                                               * thousands)
                                    / thousands;
