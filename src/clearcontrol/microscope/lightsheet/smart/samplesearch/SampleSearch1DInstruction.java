                                       ...
                                       implements LoggingFeature
{
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    for (int i = 0; i < lTravelPathList.size() - 1; i++)
    {
      ...
      steps = (int) (distance / mStepSizeInMillimetersVariable.get()
                     + 1);
      info("FOV can be travelled through with " + steps + " steps");

      double stepX = (lEndPosition.mX - lStartPosition.mX)
                     / (steps - 1);
      double stepY = (lEndPosition.mY - lStartPosition.mY)
                     / (steps - 1);
      double stepZ = (lEndPosition.mZ - lStartPosition.mZ)
                     / (steps - 1);

      for (int s = 0; s < steps; s++)
      {
        Position lPosition =
                           new Position(lStartPosition.mX + s * stepX,
    ...
    for (int i = 0; i < lTravelPathList.size() - 1; i++)
    {
      ...
      for (int s = 0; s < steps; s++)
      {
        ...
        if (count > 0
            && lAverageSignalIntensities.get(count
                                             - 1) > lAverageSignalIntensities.get(count))
        {
          localMaximum = false;
        }
        if (count < lAverageSignalIntensities.size() - 1
            && lAverageSignalIntensities.get(count
                                             + 1) > lAverageSignalIntensities.get(count))
        {
          localMaximum = false;
