                                       ...
                                       SampleSearchInstructionBase
{
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    Position lEndPosition = lTravelPathList.get(lTravelPathList.size()
                                                - 1);

    double distance = Math.sqrt(
                                Math.pow(lStartPosition.mX
                                         - lEndPosition.mX, 2)
                                + Math.pow(lStartPosition.mY
                                           - lEndPosition.mY, 2)
                                + Math.pow(lStartPosition.mZ
                                           - lEndPosition.mZ, 2));

    int steps = (int) (distance / mStepSizeInMillimetersVariable.get()
                       + 1);

    double stepX =
                 (lEndPosition.mX - lStartPosition.mX) / (steps - 1);
    double stepY =
                 (lEndPosition.mY - lStartPosition.mY) / (steps - 1);
    double stepZ =
                 (lEndPosition.mZ - lStartPosition.mZ) / (steps - 1);
    ...
    for (int sx = 0; sx < steps; sx++)
    {
      ...
      {
        Position lPosition =
                           new Position(lStartPosition.mX
                                        + sx * stepX,
                                        lStartPosition.mY
                                                      + sy * stepY,
                                        lStartPosition.mZ + (sx + sy)
                                                            / 2.0
                                                            * stepZ);
    ...
    SpaceTravelInstruction lSpaceTravelScheduler =
                                                 getLightSheetMicroscope().getDevice(SpaceTravelInstruction.class,
                                                                                     0);
    ArrayList<Position> lDetectedSamplesPositionList =
                                                     lSpaceTravelScheduler.getTravelPathList();
    lDetectedSamplesPositionList.clear();

    ArrayList<Double> lAverageSignalIntensities =
    ...
    for (int sx = 1; sx < steps - 1; sx++)
    {
      for (int sy = 1; sy < steps - 1; sy++)
      {
        ...
        if ((lAverageSignalIntensities.get(sx * steps
                                           + sy) < lAverageSignalIntensities.get(sx
                                                                                 * steps
                                                                                 + sy
                                                                                 + 1))
            || (lAverageSignalIntensities.get(sx * steps
                                              + sy) < lAverageSignalIntensities.get(sx
                                                                                    * steps
                                                                                    + sy
                                                                                    - 1))
        ...
        if (localMaximum)
        {
          lDetectedSamplesPositionList.add(mSampleCandidates.getTravelPathList()
                                                            .get(sx
                                                                 * steps
                                                                 + sy));
        }
