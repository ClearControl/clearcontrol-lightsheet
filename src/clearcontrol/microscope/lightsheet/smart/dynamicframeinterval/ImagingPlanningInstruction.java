...
/**
 * The ImagingPlanningInstruction looks in the most recent measurements from
 * image stacks and decides if frame interval should be de- or increased.
 *
 *
 * Author: @haesleinhuepf 05 2018
 */
public class ImagingPlanningInstruction extends
                                        ...
                                        implements LoggingFeature
{
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    ArrayList<SpotCountContainer> lSpotcountContainers =
                                                       lDataWarehouse.getContainers(SpotCountContainer.class);
    ...
    appendImagingSequence(pTimePoint,
                          lNumberOfImages,
                          lFrameIntervalInSeconds);
    addSpotDetectionAfterEveryFutureFusion(pTimePoint);

    return true;
  }

  private boolean appendImagingSequence(long pTimePoint,
                                        int pNumberOfImages,
                                        double pFrameIntervalInSeconds)
  {
    InstructionInterface lScheduler = null;
    if (pFrameIntervalInSeconds < 15)
    {
      lScheduler =
                 new AppendConsecutiveHyperDriveImagingInstruction(pNumberOfImages,
                                                                   pFrameIntervalInSeconds,
                                                                   getLightSheetMicroscope());
    }
    else if (pFrameIntervalInSeconds < 30)
    {
      lScheduler =
                 new AppendConsecutiveOpticsPrefusedImagingInstruction(pNumberOfImages,
                                                                       pFrameIntervalInSeconds,
                                                                       getLightSheetMicroscope());
    }
    else
    {
      lScheduler =
                 new AppendConsecutiveInterleavedImagingInstruction(pNumberOfImages,
                                                                    pFrameIntervalInSeconds,
                                                                    getLightSheetMicroscope());
    }
  ...
  private void addSpotDetectionAfterEveryFutureFusion(long pTimePoint)
  {
    ...
    for (int i = (int) pTimePoint; i < schedule.size() - 1; i++)
    {
      ...
      if ((lScheduler instanceof AbstractAcquistionInstruction)
          && (!(lFollowingScheduler instanceof CountsSpotsInstruction)))
      {
        schedule.add(i + 1,
