 ...
 *
 * This instructions analyses recent DCTS2D measurements retrieved from the
 * DataWarehouse. It checks if the following conditions are met: * A sample has
 * quality above average (compared to all samples) * The quality at time points
 * t-1, t-2, t-3 has a five fold smaller gradient than at time points t-4, t-5
 * and t-6 (on average)
 *
 * If these conditions are met, all spatial positions are removed from the
 * SpaceTravelInstruction but the one which fulfilled the conditions. Then,
 * imaging the single sample can continue.
 *
 * Author: @haesleinhuepf 05 2018
 */
public class DrosophilaSelectSampleJustBeforeInvaginationInstruction extends
                                                                     LightSheetMicroscopeInstructionBase
                                                                     implements
                                                                     LoggingFeature
{

  private BoundedVariable<Double> mDerivativeFactorVariable =
                                                            new BoundedVariable<Double>("derivative factor",
                                                                                        3.0,
                                                                                        0.0,
                                                                                        Double.MAX_VALUE,
                                                                                        0.01);
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    if (lQualityInSpaceList.size() == 0)
    {
      getLightSheetMicroscope().getTimelapse()
                               .log("No measurements found. Measure DCTS2D before asking me where the best sample is.");
    ...
    {
      getLightSheetMicroscope().getTimelapse()
                               .log("There is no decision to make. Leaving.");
    ...
    double[][] lQualityGroupedBySample =
                                       new double[lPositionList.size()][lQualityInSpaceList.size()
                                                                        / lPositionList.size()];

    // DCTS2DContainer lMaxmimumQualityContainer = lQualityInSpaceList.get(0);
    for (DCTS2DContainer lContainer : lQualityInSpaceList)
    {
      // if (lContainer.getMeasurement() >
      // lMaxmimumQualityContainer.getMeasurement()) {
      // lMaxmimumQualityContainer = lContainer;
      // }

      int positionIndex =
                        getClosestPositionIndex(lPositionList,
                                                new Position(lContainer.getX(),
                                                             lContainer.getY(),
                                                             lContainer.getZ()));
      lQualityGroupedBySample[positionIndex][qualityMeasurementSizes[positionIndex]] =
                                                                                     lContainer.getMeasurement();
    ...
    {
      maxTimePoints = Math.min(maxTimePoints,
                               qualityMeasurementSizes[i]);

      getLightSheetMicroscope().getTimelapse().log("Q[" + i
                                                   + "]: "
                                                   + Arrays.toString(qualityMeasurementSizes));
    }
    ...
    for (int i = 0; i < qualityMeasurementSizes.length; i++)
    {
      if (sampleQualityAboveMean(lQualityGroupedBySample,
                                 i,
                                 maxTimePoints - 1))
      {
        if (sampleIsJustBeforeInvagination(lQualityGroupedBySample[i],
                                           maxTimePoints))
        {
          // lBestPosition = lPositionList.get(i);
          info("Sample at position " + lPositionList.get(i)
               + " is apparently undergoing invagiation soon!");
          getLightSheetMicroscope().getTimelapse()
                                   .log("Sample at position "
                                        + lPositionList.get(i)
                                        + " is apparently undergoing invagiation soon!");
        ...
        {
          info("Sample at position " + lPositionList.get(i)
               + " is NOT undergoing invagiation soon!");
          getLightSheetMicroscope().getTimelapse()
                                   .log("Sample at position "
                                        + lPositionList.get(i)
                                        + " is NOT undergoing invagiation soon!");
    ...
    {
      info("Chosen position was " + lChosenPosition.mX
           + "/"
           + lChosenPosition.mY
           + "/"
           + lChosenPosition.mZ
  ...
  private boolean sampleQualityAboveMean(double[][] pQualityGroupedBySample,
                                         int pSampleIndex,
                                         int pTimepoint)
  {
    double[] lQualityMeasurements =
                                  new double[pQualityGroupedBySample.length];
    for (int i = 0; i < lQualityMeasurements.length; i++)
    {
      lQualityMeasurements[i] =
                              pQualityGroupedBySample[i][pTimepoint];
    }

    double lMeanQuality = new Mean().evaluate(lQualityMeasurements);
    double lQuality =
                    pQualityGroupedBySample[pSampleIndex][pTimepoint];
  ...
  private boolean sampleIsJustBeforeInvagination(double[] pQualityMeasurements,
                                                 int maxTimePoints)
  {

    double averageLastThree = 1.0 / 3.0
                              * (Math.abs(pQualityMeasurements[maxTimePoints
                                                               - 1])
                                 + Math.abs(pQualityMeasurements[maxTimePoints
                                                                 - 2])
                                 + Math.abs(pQualityMeasurements[maxTimePoints
    ...
    double averageLastThreeDerivative = 1.0 / 3.0
                                        * (Math.abs(qualityDerivative[maxTimePoints
                                                                      - 2])
                                           + Math.abs(qualityDerivative[maxTimePoints
                                                                        - 3])
                                           + Math.abs(qualityDerivative[maxTimePoints
    ...
    // In case of a soon to start invagination we assume quality to be mostly
    // stable. Furthermore, in the past,
    // quality is supposed to have changed a lot. Thus quality deltas should be
    // very different (by a factor):
    double factor = averageThreeBeforeDerivative
                    / averageLastThreeDerivative;
    // info("quality: " + Arrays.toString(pQualityMeasurements));
    // info("derivative: " + Arrays.toString(qualityDerivative));
    info("avg der " + averageThreeBeforeDerivative
         + " "
         + averageLastThreeDerivative
         + " avg "
         + averageThreeBefore
         ...
         + averageLastThree);
    info("Factor: " + factor);
    return (factor > getDerivativeFactorVariable().get())
           && (averageThreeBefore < averageLastThree);
  }

  private int getClosestPositionIndex(ArrayList<Position> lPositionList,
                                      Position lPosition)
  {
    ...
    for (int i = 0; i < lPositionList.size(); i++)
    {
      ...
      double distance = Math.sqrt(
                                  Math.pow(lPosition.mX
                                           - lPositionOutOfList.mX,
                                           2.0)
                                  + Math.pow(lPosition.mY
                                             - lPositionOutOfList.mY,
                                             2.0)
                                  + Math.pow(lPosition.mZ
                                             - lPositionOutOfList.mZ,
                                             2.0));
  ...
  {
    DrosophilaSelectSampleJustBeforeInvaginationInstruction copied =
                                                                   new DrosophilaSelectSampleJustBeforeInvaginationInstruction(getLightSheetMicroscope());
