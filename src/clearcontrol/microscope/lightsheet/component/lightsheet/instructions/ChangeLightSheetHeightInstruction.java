...
/**
 * ChangeLightSheetHeightInstruction allows controlling the scan head to change
 * the height of light sheets
 *
 * Author: @haesleinhuepf July 2018
 */
public class ChangeLightSheetHeightInstruction extends
                                               LightSheetMicroscopeInstructionBase
{

  private final BoundedVariable<Integer> mLightSheetIndex;
  private final BoundedVariable<Double> mLightSheetHeight =
                                                          new BoundedVariable<Double>("Light sheet height",
                                                                                      0.0,
                                                                                      -Double.MAX_VALUE,
                                                                                      Double.MAX_VALUE,
                                                                                      0.01);

  public ChangeLightSheetHeightInstruction(LightSheetMicroscope pLightSheetMicroscope,
                                           int pLightSheetIndex,
                                           double pLightSheetHeight)
  {
    super("Adaptation: Change light sheet height to "
          + pLightSheetHeight, pLightSheetMicroscope);
    ...
    for (int cpi = 0; cpi < lState.getNumberOfControlPlanes(); cpi++)
    {
      for (int l = 0; l < lState.getNumberOfLightSheets(); l++)
      {
        lState.getInterpolationTables()
              .set(LightSheetDOF.IH, cpi, l, mLightSheetHeight.get());
      }
    }
    return true;
  }
  ...
  {
    return new ChangeLightSheetHeightInstruction(getLightSheetMicroscope(),
