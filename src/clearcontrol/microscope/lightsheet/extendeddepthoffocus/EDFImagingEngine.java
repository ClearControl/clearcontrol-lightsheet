...
/**
 * The EDFImagingEngine allows taking images with several focus planes per light
 * sheet position and vice versa.
 *
 * Initially, n light sheet positions are imaged; with m detection arm positions
 * each. Afterwards, within the m images, the one with the highest quality
 * measurement is defined as the one in focus. In the following step, again m
 * detection arm positons are imaged, but the range becomes smaller. After 3-4
 * iterations, the minimum range should be reached. Adaption according to the
 * detection arm position in focus is done at every single step.
 *
                              ...
                              VisualConsoleInterface
{
  ...
  private final BoundedVariable<Double> mMinimumRange =
  ...
  private BoundedVariable<Integer> mNumberOfIterations =
                                                       new BoundedVariable<Integer>("Number of iterations",
                                                                                    20,
                                                                                    0,
                                                                                    Integer.MAX_VALUE,
                                                                                    1);

  private final BoundedVariable<Double> mExposureTimeForEDFInSeconds =
  ...
  private final BoundedVariable<Double> mExposureTimeForStacksInSeconds =
  ...
  private final Variable<Boolean> mSaveEDFStacks =
                                                 new Variable<Boolean>("Save EDF stacks",
                                                                       false);
  private final Variable<Boolean> mSaveCameraStacks =
                                                    new Variable<Boolean>("Save camera stacks",
                                                                          false);
  private final Variable<Boolean> mSaveFusedStacks =
                                                   new Variable<Boolean>("Save fused stacks",
                                                                         true);

  private BufferedWriter mLogFileWriter;

  private Variable<String> mDataSetNamePostfixVariable =
                                                       new Variable<String>("Test");
  private Variable<File> mRootFolderVariable =
                                             new Variable("RootFolder",
                                                          (Object) null);

  private Variable<Boolean> mDetectionArmFixedVariable =
                                                       new Variable<Boolean>("DetectionArmFixed",
                                                                             false);

  private BoundedVariable<Integer> mNumberOfStackSlicesVariable =
                                                                new BoundedVariable<Integer>("Number of slices",
                                                                                             100,
                                                                                             0,
                                                                                             Integer.MAX_VALUE,
                                                                                             1);
  ...
  public EDFImagingEngine(ClearCLContext pContext,
                          LightSheetMicroscope pLightSheetMicroscope)
  {
    ...
    mFastFusionEngine =
                      new LightSheetFastFusionEngine(pContext,
                                                     null,
                                                     1,
    ...
    BoundedVariable<Number> lLightsheetZVariable =
                                                 lLightSheetDevice.getZVariable();

    mFirstZ = new BoundedVariable<Double>("First Z",
                                          lLightsheetZVariable.getMin()
                                                              .doubleValue(),
                                          lLightsheetZVariable.getMin()
                                                              .doubleValue(),
                                          lLightsheetZVariable.getMax()
                                                              .doubleValue(),
                                          1.0);
  ...
  private boolean image() throws InterruptedException,
                          ExecutionException,
                          TimeoutException,
                          IOException
  {
    ...
    int lNumberOfIterations = mNumberOfIterations.get();

    long lImageWidth = (Long) getLightSheetMicroscope()
                                                       .getDevice(StackCameraDeviceInterface.class,
                                                                  0)
                                                       .getStackWidthVariable()
                                                       .get();
    long lImageHeight = (Long) getLightSheetMicroscope()
                                                        .getDevice(StackCameraDeviceInterface.class,
                                                                   0)
                                                        .getStackHeightVariable()
                                                        .get();

    double lExposureTimeForEDFInSeconds =
                                        mExposureTimeForEDFInSeconds.get();
    double lExposureTimeForStacksInSeconds =
                                           mExposureTimeForStacksInSeconds.get();

    String lDatasetname =
                        "" + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-00-").format(new Date())
    ...
    File lLogFile = new File(mRootFolderVariable.get() + "\\"
                             + lDatasetname,
                             "log.txt");
    ...
    double lMinFixedZ = mFirstZ.get(); // lDetectionFocusZVariable.getMin().doubleValue();
    double lMaxFixedZ = mLastZ.get(); // lDetectionFocusZVariable.getMax().doubleValue();
    ...
    ImageRange[][] lImageRanges =
                                new ImageRange[lNumberOfDetectionArms][lNumberOfFixedSamples];

    double lStepMovingZ = (lMaxMovingZ - lMinMovingZ)
                          / (lNumberOfMovingSamples - 1);
    double lStepFixedZ = (lMaxFixedZ - lMinFixedZ)
                         / (lNumberOfFixedSamples - 1);

    int lNumberOfStackSlices = mNumberOfStackSlicesVariable.get();

    double lQualityStandardDeviationMultiplier = 1;

    mLogFileWriter.write("Dataset " + mRootFolderVariable.get()
                         + "\\"
                         + lDatasetname);
    mLogFileWriter.write("Number of "
                         + (mDetectionArmFixedVariable.get() ? "detection arm"
                                                             : "light sheet")
                         + " samples: "
                         + lNumberOfFixedSamples);
    mLogFileWriter.write("Number of "
                         + (!mDetectionArmFixedVariable.get() ? "detection arm"
                                                              : "light sheet")
                         + " samples: "
                         + lNumberOfMovingSamples);
    mLogFileWriter.write("Detection arm fixed: "
                         + mDetectionArmFixedVariable.get());
    mLogFileWriter.write("Number of iterations: "
                         + lNumberOfIterations);
    ...
    mLogFileWriter.write("Exposure time EDF (sec): "
                         + lExposureTimeForEDFInSeconds);
    mLogFileWriter.write("Exposure time Stacks (sec): "
                         + lExposureTimeForStacksInSeconds);
    mLogFileWriter.write("Number of slices in stacks: "
                         + lNumberOfStackSlices);

    mLogFileWriter.write("Start "
                         + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS-").format(new Date())
                         + "\n");

    OffHeapPlanarStack[] lNormalStacks =
                                       new OffHeapPlanarStack[lNumberOfDetectionArms];

    OffHeapPlanarStack[] lEDFStacks =
                                    new OffHeapPlanarStack[lNumberOfDetectionArms];

    // define ImageRange: For all detection arm positions take
    // images for all light sheet positions (or vise versa)

    for (int lDetectionArm =
                           0; lDetectionArm < lNumberOfDetectionArms; lDetectionArm++)
    {

      int lQualitySampleIndex = 0;
      for (double lFixedZ =
                          lMinFixedZ; lFixedZ <= lMaxFixedZ
                                                 + 0.0001; lFixedZ +=
                                                                   lStepFixedZ)
      {
        ...
        int lMovingCount = 0;
        for (double lMovingZ =
                             lMinMovingZ; lMovingZ <= lMaxMovingZ
                                                      + 0.00001; lMovingZ +=
                                                                          lStepMovingZ)
        {
      ...
      // setup imager
      FocusableImager lInitialImager =
      ...
      lEDFStacks[lDetectionArm] =
                                imageEDFStack(lInitialImager,
                                              lImageRanges[lDetectionArm]);
    }
    ...
    BasicRecycler<StackInterface, StackRequest> stackRecycler =
                                                              new BasicRecycler(new ContiguousOffHeapPlanarStackFactory(),
                                                                                ...
                                                                                10,
                                                                                true);
    StackInterface lFusedStack = null;

    for (int lDetectionArm =
                           0; lDetectionArm < lNumberOfDetectionArms; lDetectionArm++)
    {
      for (int lFixedPosition =
                              0; lFixedPosition < lNumberOfFixedSamples; lFixedPosition++)
      {
        configureChart("Quality",
                       "Quality_C" + lDetectionArm
                                  + "_CPI"
                                  + lFixedPosition,
                       "EDF slice",
                       "Quality",
      ...
      configureChart("Quality",
                     ...
                     "Quality",
                     ChartType.Line);
      configureChart("Selected_Z",
                     "Selected_Z_C" + lDetectionArm,
                     "fixedZ",
                     "movingZ",
                     ChartType.Line);
    ...
    while (true)
    {
                             ...
                             0; lDetectionArm < lNumberOfDetectionArms; lDetectionArm++)
      {
        ...
        mLogFileWriter.write("Imaging with detection arm "
                             + lDetectionArm + "\n");
        ...
        if (mSaveEDFStacks.get())
        {
          ...
          lEDFStacks[lDetectionArm].getMetaData().addEntry(
                                                           MetaDataChannel.Channel,
                                                           lChannel);
          mLogFileWriter.write("Start writing EDF stack to disc "
                               + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                               + "\n");
          lSink.appendStack(lChannel, lEDFStacks[lDetectionArm]);
          mLogFileWriter.write("Stop writing EDF stack to disc  "
                               + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                               + "\n");
        }
        ...
        mLogFileWriter.write("Start analysing quality "
                             + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                             + "\n");
        DiscreteConsinusTransformEntropyPerSliceEstimator lImageQualityEstimator =
                                                                                 new DiscreteConsinusTransformEntropyPerSliceEstimator(lEDFStacks[lDetectionArm]);
        double[] lQualityPerSliceMeasurementsArray =
                                                   lImageQualityEstimator.getQualityArray();

        mLogFileWriter.write("Stop analysing quality  "
                             + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                             + "\n");
        ...
        double lMeanQuality =
                            new Mean().evaluate(lQualityPerSliceMeasurementsArray);
        double lStandardDeviationQuality =
                                         new StandardDeviation().evaluate(lQualityPerSliceMeasurementsArray,
                                                                          lMeanQuality);

        double lQualityThreshold = lMeanQuality
                                   + lStandardDeviationQuality
                                     * lQualityStandardDeviationMultiplier;
        ...
        addPoint("Quality",
                 "Quality_C" + lDetectionArm + "_threshold",
                 lClearQualityGraph,
                 lQualityPerSliceMeasurementsArray.length - 1,
                 lQualityThreshold);
        ...
        for (ImageRange lImageRange : lImageRanges[lDetectionArm])
        {
          ...
          lFixedPosition++;
          // configureChart("Selected_Z", "Selected_Z_C" + lDetectionArm, "LZ",
          // "DZ", ChartType.Scatter);

          addPoint("Selected_Z",
                   "Selected_Z_C" + lDetectionArm,
                   lClearSelectedZGraph,
                   lImageRange.mFixedPosition,
                   lBestMovingZ);
          lClearSelectedZGraph = false;

          mLogFileWriter.write("Best moving position " + lBestMovingZ
                               + " Quality: "
                               + lMaxQuality
                               + "\n");
          if (lMaxQuality < lQualityThreshold)
          {
            warning("Quality in CPI " + lFixedPosition
                    + " to low! Resetting to calibration.");
            lBestMovingZ = lImageRange.mFixedPosition;
          }

          // setup a new range for this particular fixed slice
          double lOldRange =
                           lImageRange.mMovingPositions[lNumberOfMovingSamples
                                                        - 1]
          ...
          mLogFileWriter.write("Fixed " + lImageRange.mFixedPosition
                               + " new moving range "
          ...
          int lMovingCount = 0;
          for (double lMovingZ = lBestMovingZ
                                 - lNewRange
                                   / 2; lMovingZ <= lBestMovingZ
                                                    + lNewRange / 2
                                                    + sDoubleTolerance; lMovingZ +=
                                                                                 lStepMovingZ)
          {
            lImageRange.mMovingPositions[lMovingCount] =
                                                       Math.max(Math.min(lMovingZ,
                                                                         lMaxMovingZ),
                                                                lMinMovingZ);
            lMovingCount++;
          }
        ...
        mLogFileWriter.write("ITERATION "
        ...
        // take new images
        FocusableImager lPreciseImager =
                                       new FocusableImager(getLightSheetMicroscope(),
                                                           lLightSheetMinIndex,
                                                           lLightSheetMaxIndex,
                                                           lDetectionArmCopy,
        ...
        try
        {
          ...
          lEDFStacks[lDetectionArmCopy] =
                                        imageEDFStack(lPreciseImager,
                                                      lImageRanges[lDetectionArmCopy]);

          info("Imaging C" + lDetectionArm + "...");
          lNormalStacks[lDetectionArmCopy] =
                                           imageNormalStack(lImager,
                                                            lImageRanges[lDetectionArmCopy],
                                                            lNumberOfStackSlices);
        }
        catch (InterruptedException e)
        ...
        if (mSaveCameraStacks.get())
        {
          ...
          mLogFileWriter.write("Start writing stack to disc "
                               + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                               + "\n");
          lSink.appendStack(lNormalChannel,
                            lNormalStacks[lDetectionArmCopy]);
          mLogFileWriter.write("Stop writing stack to disc  "
                               + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                               + "\n");
        }
        /*
      ...
      {
        info("Fusing images...");
        mLogFileWriter.write("Start fusing   "
                             + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                             + "\n");
                               ...
                               0; lDetectionArm < lNumberOfDetectionArms; lDetectionArm++)
        {
          ...
          mFastFusionEngine.passImage("C" + lDetectionArm
                                      + "L0",
                                      lNormalStacks[lDetectionArm].getContiguousMemory(),
                                      ImageChannelDataType.UnsignedInt16,
                                      lNormalStacks[lDetectionArm].getDimensions());
        }
        // mFastFusionEngine.addTask(new MemoryReleaseTask("fused", "C0L0",
        // "C1L0", "C0L0d", "C1L0d", "C0L0blur", "C1L0blur", "C1L0reg"));
        ...
        ClearCLImage lFusedImage =
                                 mFastFusionEngine.getImage("fused");
        ...
        mLogFileWriter.write("Stop fusing    "
                             + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                             + "\n");

        info("Saving fused stack...");
        mLogFileWriter.write("Start writing fused stack to disc "
                             + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                             + "\n");
  ...
  private OffHeapPlanarStack imageNormalStack(FocusableImager pImager,
                                              ImageRange[] pImageRanges,
                                              int pNumberOfSlices) throws InterruptedException,
                                                                   ExecutionException,
                                                                   TimeoutException,
                                                                   IOException
  {
    ...
    UnivariateInterpolator lLightSheetPositionInterpolator =
                                                           new SplineInterpolator();
    UnivariateFunction lLightSheetPositionFunction =
                                                   lLightSheetPositionInterpolator.interpolate(x,
                                                                                               yLightSheetPosition);

    UnivariateInterpolator lDetectionArmPositionInterpolator =
                                                             new SplineInterpolator();
    UnivariateFunction lDetectionArmPositionFunction =
                                                     lDetectionArmPositionInterpolator.interpolate(x,
                                                                                                   yDetectionArmPosition);

    for (int slice = 0; slice < pNumberOfSlices; slice++)
    {
      double position = (double) (count - 1) * slice
                        / (pNumberOfSlices);
    ...
    */
    ...
    mLogFileWriter.write("Start imaging    "
                         + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                         + "\n");
    final OffHeapPlanarStack lStack = pImager.execute();
    mLogFileWriter.write("Finished imaging "
                         + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                         + "\n");

    lStack.getMetaData().addEntry(MetaDataView.Camera,
                                  pImager.getDetectionArmIndex());
    lStack.getMetaData().addEntry(
                                  MetaDataAcquisitionType.AcquisitionType,
  ...
  private OffHeapPlanarStack imageEDFStack(FocusableImager pImager,
                                           ImageRange[] pImageRanges) throws InterruptedException,
                                                                      ExecutionException,
                                                                      TimeoutException,
                                                                      IOException
  {
    ...
    for (ImageRange lImageRange : pImageRanges)
    {
      for (double lMovingPosition : lImageRange.mMovingPositions)
      {
        ...
        if (mDetectionArmFixedVariable.get())
        {
          mLogFileWriter.write("Image at " + lMovingPosition
                               + "/"
                               + lImageRange.mFixedPosition
                               + "\n");
          pImager.addImageRequest(lMovingPosition,
    ...
    // Actually run the imaging here
    mLogFileWriter.write("Start EDF imaging    "
                         + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                         + "\n");
    final OffHeapPlanarStack lStack = pImager.execute();
    mLogFileWriter.write("Finished EDF imaging "
                         + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SSS").format(new Date())
                         + "\n");
