...
/**
 * The MeasureDCTS2DOnStackInstruction measures average image quality of a
 * stack, puts the result in the data warehouse together with spatial (stage)
 * position, timepoint. Furthermore, the measurement is saved in a TSV log file.
 *
 * Author: @haesleinhuepf 05 2018
 */
public class MeasureDCTS2DOnStackInstruction<T extends StackInterfaceContainer>
                                            extends
                                            ...
                                            implements LoggingFeature
{
  ...
  {
    super("Post-processing: DCTS2D measurement for "
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    ElapsedTime.measure("dcts2d determination", () -> {
      ...
      DiscreteConsinusTransformEntropyPerSliceEstimator lDCTS2DEstimator =
                                                                         new DiscreteConsinusTransformEntropyPerSliceEstimator(lStack);
      double[] lQualityPerslice = lDCTS2DEstimator.getQualityArray();

      double lMeanDCTS2DQuality =
                                new Mean().evaluate(lQualityPerslice);
      ...
      DCTS2DContainer lDCTS2DContainer =
                                       new DCTS2DContainer(pTimePoint,
                                                           lX,
                                                           lY,
                                                           lZ,
                                                           lMeanDCTS2DQuality);
      SliceBySliceDCTS2DContainer lSliceBySliceDCTS2DContainer =
                                                               new SliceBySliceDCTS2DContainer(pTimePoint,
                                                                                               lQualityPerslice);

      getLightSheetMicroscope().getDataWarehouse()
                               .put("DCTS2D_" + pTimePoint,
                                    lDCTS2DContainer);
      getLightSheetMicroscope().getDataWarehouse().put(
                                                       "SliceBySliceDCTS2D_"
                                                       + pTimePoint,
                                                       lSliceBySliceDCTS2DContainer);
      ...
      int[] lSliceNumber =
                         new IntRange(0,
                                      lQualityPerslice.length).toArray();
      String sliceBySliceHeadline = "t\tX\tY\tZ"
                                    + (Arrays.toString(lSliceNumber)
                                             .replace(", ", "\t")
                                             .replace("[", "")
                                             .replace("]", ""))
                                    + "\n";
      String sliceBySliceResultTableLine = pTimePoint + "\t"
                                           + lX
                                           + "\t"
                                           + lY
                                           + "\t"
                                           + lZ
                                           + "\t"
                                           + Arrays.toString(lQualityPerslice)
                                                   .replace(", ",
                                                            "\t")
                                                   .replace("[", "")
                                                   .replace("]", "")
      ...
      File lSliceBySliceOutputFile =
                                   new File(targetFolder
                                            + "/SliceBySliceDCTS2d.tsv");
      try
      {
      ...
      try
      {
        ...
        BufferedWriter writer =
                              new BufferedWriter(new FileWriter(lSliceBySliceOutputFile,
                                                                true));
        if (!existedBefore)
        {
  ...
  {
    return new MeasureDCTS2DOnStackInstruction(mClass,
