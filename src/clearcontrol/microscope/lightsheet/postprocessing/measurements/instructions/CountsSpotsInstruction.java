...
/**
 * The CountsSpotsInstruction takes the image stack out of a the olderst of a
 * given container type and counts spots in the data set. The number of spots is
 * then saved to a txt file in the current working directory.
 *
 * Author: @haesleinhuepf 04 2018
 */
public class CountsSpotsInstruction<T extends StackInterfaceContainer>
                                   extends
                                   ...
                                   PropertyIOableInstructionInterface
{
  ...
  BoundedVariable<Double> mThreshold =
                                     new BoundedVariable<Double>("threshold",
                                                                 200.0,
                                                                 0.0,
                                                                 Double.MAX_VALUE,
                                                                 0.1);
  BoundedVariable<Double> mXYDownsamplingFactor =
                                                new BoundedVariable<Double>("downsample XY by ",
                                                                            0.25,
                                                                            0.0,
                                                                            Double.MAX_VALUE,
                                                                            0.001);
  BoundedVariable<Double> mZDownsamplingFactor =
                                               new BoundedVariable<Double>("downsample Z by ",
                                                                           1.0,
                                                                           0.0,
                                                                           Double.MAX_VALUE,
                                                                           0.001);
  Variable<Boolean> mShowIntermediateResults =
                                             new Variable<Boolean>("show intermediate results",
                                                                   false);
  BoundedVariable<Double> mDoGSigmaMinued =
                                          new BoundedVariable<Double>("DoG sigma minuend",
                                                                      3.0,
                                                                      0.0,
                                                                      Double.MAX_VALUE,
                                                                      0.1);
  BoundedVariable<Double> mDoGSigmaSubtrahend =
                                              new BoundedVariable<Double>("DoG sigma subtrahend",
                                                                          6.0,
                                                                          0.0,
                                                                          Double.MAX_VALUE,
                                                                          0.1);
  BoundedVariable<Double> mBlurSigma =
                                     new BoundedVariable<Double>("Blur sigma",
                                                                 6.0,
                                                                 0.0,
                                                                 Double.MAX_VALUE,
                                                                 0.1);
  BoundedVariable<Integer> mDoGRadius =
                                      new BoundedVariable<Integer>("DoG radius",
                                                                   3,
                                                                   0,
                                                                   Integer.MAX_VALUE);
  BoundedVariable<Integer> mBlurRadius =
                                       new BoundedVariable<Integer>("Blur radius",
                                                                    12,
                                                                    0,
                                                                    Integer.MAX_VALUE);
  BoundedVariable<Integer> mOptimaDetectionRadiud =
                                                  new BoundedVariable<Integer>("Optima detection radius",
                                                                               6,
                                                                               0,
                                                                               Integer.MAX_VALUE);
  ...
  {
    super("Post-processing: Spot detection for "
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    ClearCLImage lSpotsImage = clij.createCLImage(new long[]
    { (long) (lCLImage.getWidth() * mXYDownsamplingFactor.get()),
      (long) (lCLImage.getHeight() * mXYDownsamplingFactor.get()),
      (long) (lCLImage.getDepth() * mZDownsamplingFactor.get()) },
                                                  lCLImage.getChannelDataType());
    // ClearCLImage lCLMaximumProjectionImage = clij.createCLImage(new
    // long[]{lCLImage.getWidth(), lCLImage.getHeight()},
    // lCLImage.getChannelDataType());

    // Kernels.maxProjection(clij, lCLImage, lCLMaximumProjectionImage);

    // ImagePlus lImpMaximumProjection =
    // clij.converter(lCLMaximumProjectionImage).getImagePlus();
    // lCLImage.close();
    ...
    ElapsedTime.measure("spot detection ", () -> {
      ...
      GPUSpotDetection lGPUSpotDetector =
                                        new GPUSpotDetection(clij,
                                                             input,
                                                             output,
                                                             mThreshold.get()
                                                                       .floatValue());
      ...
      lGPUSpotDetector.setDogSigmaMinuend(mDoGSigmaMinued.get()
                                                         .floatValue());
      lGPUSpotDetector.setDogSigmaSubtrahend(mDoGSigmaSubtrahend.get()
                                                                .floatValue());
      ...
      StackInterface lSpotsStack = clij.converter(lSpotsImage)
                                       .getOffHeapPlanarStack();
      SpotsImageContainer lSpotsImageContainer =
                                               new SpotsImageContainer(pTimePoint);
      lSpotsImageContainer.put("spots", lSpotsStack);
      getLightSheetMicroscope().getDataWarehouse()
                               .put("spots_" + pTimePoint,
                                    lSpotsImageContainer);
      ...
      SpotCountContainer lSpotCountContainer =
                                             new SpotCountContainer(pTimePoint,
                                                                    lX,
                                                                    lY,
                                                                    lZ,
                                                                    lSpotCount);
      ...
      getLightSheetMicroscope().getDataWarehouse()
                               .put("SPOTCOUNT_" + pTimePoint,
                                    lSpotCountContainer);

      String headline = "t\tX\tY\tZ\tspotcount\n";
      String resultTableLine = pTimePoint + "\t"
                               + lX
                               + "\t"
                               + lY
                               + "\t"
    ...
    // IJ.saveAs(lImpMaximumProjection, "JPEG", targetFolder +
    // "/stacks/thumbnails/" + String.format("%0" + lDigits + "d", lTimePoint) +
    // ".jpg");
    return true;
  }
  ...
  {
    CountsSpotsInstruction copied =
                                  new CountsSpotsInstruction(mClass,
