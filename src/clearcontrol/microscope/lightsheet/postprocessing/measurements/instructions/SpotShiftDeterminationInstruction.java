 ...
 */
public class SpotShiftDeterminationInstruction extends
                                               ...
                                               PropertyIOableInstructionInterface
{
  ...
  private BoundedVariable<Integer> mNumberOfDilations =
                                                      new BoundedVariable<Integer>("Number of dilations",
                                                                                   8,
                                                                                   2,
                                                                                   Integer.MAX_VALUE,
                                                                                   2);
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    ArrayList<SpotsImageContainer> spotsImageContainers =
                                                        dataWarehouse.getContainers(SpotsImageContainer.class);

    if (spotsImageContainers.size() < 2)
    {

      getLightSheetMicroscope().getTimelapse()
                               .log("Not enough spots images found!");
      warning("Not enough spots images found!");
      return false;
    }

    StackInterface lStackA = spotsImageContainers
                                                 .get(spotsImageContainers.size()
                                                      - 2)
                                                 .get("spots");
    StackInterface lStackB = spotsImageContainers
                                                 .get(spotsImageContainers.size()
                                                      - 1)
                                                 .get("spots");
    ...
    if (lStackA == null || lStackB == null)
    {
      ...
      getLightSheetMicroscope().getTimelapse()
                               .log("At least one spots image was empty!");
    ...
    ElapsedTime.measure("Sorenson-Dice determination", () -> {
      ...
      ClearCLImage lCLImageA = clij.converter(lStackA)
                                   .getClearCLImage();
      ClearCLImage lCLImageB = clij.converter(lStackB)
                                   .getClearCLImage();

      ClearCLImage flip =
                        clij.createCLImage(lCLImageA.getDimensions(),
                                           lCLImageA.getChannelDataType());
      ...
      getLightSheetMicroscope().getTimelapse()
                               .log("pixel count overlap "
                                    + pixelCountOverlap);
      getLightSheetMicroscope().getTimelapse()
                               .log("pixel count a " + pixelCountA);
      getLightSheetMicroscope().getTimelapse()
                               .log("pixel count b " + pixelCountB);

      double diceIndex = 2.0 * pixelCountOverlap
                         / (pixelCountA + pixelCountB);
      getLightSheetMicroscope().getTimelapse().log(
                                                   "pixel count dice index "
                                                   + diceIndex);

      SorensonDiceIndexContainer lSorensonDiceIndexcontainer =
                                                             new SorensonDiceIndexContainer(pTimePoint,
                                                                                            diceIndex);
      dataWarehouse.put("dice_" + pTimePoint,
                        lSorensonDiceIndexcontainer);
