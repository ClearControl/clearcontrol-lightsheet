 ...
 */
public class HalfStackMaxProjectionInstructionPanel extends
                                                    CustomGridPane
{
  public HalfStackMaxProjectionInstructionPanel(HalfStackMaxProjectionInstruction pHalfStackMaxProjectionInstruction)
  {
    ...
    addCheckbox(pHalfStackMaxProjectionInstruction.getPrintSequenceNameVariable(),
                1);
    addCheckbox(pHalfStackMaxProjectionInstruction.getPrintTimePointVariable(),
                2);

    StringVariableTextField lTextField =
                                       new StringVariableTextField(pHalfStackMaxProjectionInstruction.getMustContainStringVariable()
                                                                                                     .getName(),
                                                                   pHalfStackMaxProjectionInstruction.getMustContainStringVariable());
    add(lTextField.getLabel(), 0, 3);
    add(lTextField.getTextField(), 1, 3);

    addIntegerField(pHalfStackMaxProjectionInstruction.getFontSizeVariable(),
                    4);
    addDoubleField(pHalfStackMaxProjectionInstruction.getScalingVariable(),
                   5);
  }
}
