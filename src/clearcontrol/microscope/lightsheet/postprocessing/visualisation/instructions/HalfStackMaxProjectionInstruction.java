                                              ...
                                              PropertyIOableInstructionInterface
{
  ...
  private Variable<Boolean> mViewFrontVariable =
                                               new Variable<Boolean>("Front view",
                                                                     true);
  private Variable<Boolean> mPrintSequenceNameVariable =
                                                       new Variable<Boolean>("Print sequence name",
                                                                             true);
  private Variable<Boolean> mPrintTimePointVariable =
                                                    new Variable<Boolean>("Print time point",
                                                                          true);
  private Variable<String> mMustContainStringVariable =
                                                      new Variable<String>("Stack lable must contain",
                                                                           "");
  ...
  private BoundedVariable<Integer> mFontSizeVariable =
                                                     new BoundedVariable<Integer>("Font size",
                                                                                  14,
                                                                                  5,
                                                                                  Integer.MAX_VALUE);
   ...
   *
   */
  public HalfStackMaxProjectionInstruction(Class<T> pClass,
                                           boolean pViewFront,
                                           LightSheetMicroscope pLightSheetMicroscope)
  {
    super("Post-processing: Thumbnail (half stack max projection, "
          + (pViewFront ? "front" : "back")
          + ") generator for "
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    while (iterator.hasNext())
    {
      key = iterator.next();
      if (key.toLowerCase()
             .contains(mMustContainStringVariable.get().toLowerCase())
          || getMustContainStringVariable().get().length() == 0)
      {
    ...
    if (lStack == null)
    {
      warning("Couldn't find key '"
              + getMustContainStringVariable().get()
              + "' in containter "
              + lContainer
    ...
    if (lStack.getMetaData() != null)
    {
      ...
      lImagePlus.getCalibration().pixelWidth = lStack.getMetaData()
                                                     .getVoxelDimX();
      lImagePlus.getCalibration().pixelHeight = lStack.getMetaData()
                                                      .getVoxelDimY();
      lImagePlus.getCalibration().pixelDepth = lStack.getMetaData()
                                                     .getVoxelDimZ();
    }
    ...
    if (mViewFrontVariable.get())
    {
      ...
      halfStackProjectionPlugin.maxSlice =
                                         lImagePlus.getNSlices() / 2;
    }
    else
    {
      halfStackProjectionPlugin.minSlice =
                                         lImagePlus.getNSlices() / 2;
      halfStackProjectionPlugin.maxSlice =
                                         lImagePlus.getNSlices() - 1;
    }
    ...
    {
      ClearCLImage lCLImage = clij.converter(lResultImagePlus)
                                  .getClearCLImage();
      ClearCLImage lClImageScaled = clij.createCLImage(new long[]
      { (long) (lCLImage.getWidth()
                * mScalingFactorVariable.get().floatValue()),
      ...
      Kernels.downsample(clij,
                         ...
                         mScalingFactorVariable.get().floatValue());
      lResultImagePlus =
                       clij.converter(lClImageScaled).getImagePlus();
      lCLImage.close();
      ...
      if (lStack.getMetaData() != null)
      {
        ...
        lResultImagePlus.getCalibration().pixelWidth =
                                                     lStack.getMetaData()
                                                           .getVoxelDimX()
                                                       / mScalingFactorVariable.get()
                                                                               .floatValue();
        lResultImagePlus.getCalibration().pixelHeight =
                                                      lStack.getMetaData()
                                                            .getVoxelDimY()
                                                        / mScalingFactorVariable.get()
                                                                                .floatValue();
    ...
    if (lStack.getMetaData() != null)
    {
      ...
      Font font = new Font("SanSerif",
                           Font.PLAIN,
                           mFontSizeVariable.get());
      ...
      String comment = "";
      ProjectionCommentContainer commentContainer =
                                                  getLightSheetMicroscope().getDataWarehouse()
                                                                           .getOldestContainer(ProjectionCommentContainer.class);
      ...
      TimeStampContainer lStartTimeInNanoSecondsContainer =
                                                          TimeStampContainer.getGlobalTimeSinceStart(getLightSheetMicroscope().getDataWarehouse(),
                                                                                                     ...
                                                                                                     lStack);
      Duration duration =
                        Duration.ofNanos(lStack.getMetaData()
                                               .getTimeStampInNanoseconds()
                                         - lStartTimeInNanoSecondsContainer.getTimeStampInNanoSeconds());
      long s = duration.getSeconds();
      ip.drawString(String.format("%d:%02d:%02d",
                                  s / 3600,
                                  (s % 3600) / 60,
                                  (s % 60))
                    + (mPrintTimePointVariable.get() ? " (tp "
                                                       + pTimePoint
                                                       + ")"
                                                     : "")
                    + "\n"
      ...
      IJ.run(lResultImagePlus,
             "Scale Bar...",
             "width=100 height=3 font=" + mFontSizeVariable.get()
                             + " color=White background=None location=[Lower Left]");

      new File(targetFolder + "/stacks/"
               + folderName
               + "_text/").mkdirs();
      IJ.saveAsTiff(lResultImagePlus,
                    targetFolder + "/stacks/"
  ...
  {
    HalfStackMaxProjectionInstruction copied =
                                             new HalfStackMaxProjectionInstruction(mClass,
                                                                                   mViewFrontVariable.get(),
