 ...
 */
public class BasicThreeAxesStageInstructionPanel extends
                                                 CustomGridPane
{
  private int mRow = 0;
  ...
  public BasicThreeAxesStageInstructionPanel(BasicThreeAxesStageInstruction pBasicThreeAxesStageScheduler)
  {
    ...
    NumberVariableTextField<Integer> lField =
                                            new NumberVariableTextField<Integer>(pBasicThreeAxesStageScheduler.getNumberOfStepsVariable()
                                                                                                              .getName(),
                                                                                 pBasicThreeAxesStageScheduler.getNumberOfStepsVariable());
    this.add(lField.getLabel(), 0, mRow);
    this.add(lField.getTextField(), 1, mRow);
    mRow++;

    addCheckbox(pBasicThreeAxesStageScheduler.getRestartAfterFinishVariable(),
                mRow);
    mRow++;
  ...
  private void addVariableTextField(BoundedVariable<Double> pVariable)
  {
    NumberVariableTextField<Double> lField =
                                           new NumberVariableTextField<Double>(pVariable.getName(),
                                                                               pVariable);
    this.add(lField.getLabel(), 0, mRow);
    this.add(lField.getTextField(), 1, mRow);
    mRow++;
