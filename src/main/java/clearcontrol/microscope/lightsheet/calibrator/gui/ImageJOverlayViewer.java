...
public class ImageJOverlayViewer implements LoggingFeature
{
  ...
  public void show()
  {
    ...
    for (Stack2DDisplay lStack2DDisplay : mStack2DDisplays)
    {
      ...
      if (lStack != null)
      {
        RandomAccessibleInterval lRandomAccessibleInterval =
                                                           new StackToImgConverter(lStack).getRandomAccessibleInterval();
        ImagePlus lImagePlus =
                             ImageJFunctions.wrap(lRandomAccessibleInterval,
                                                  "temp");
    ...
    ImagePlus[] lImagePlusArray =
                                new ImagePlus[lImagePlusList.size()];
    lImagePlusList.toArray(lImagePlusArray);

    info("Showing " + lImagePlusList.size() + " images");
    ImagePlus fusedImage =
                         RGBStackMerge.mergeChannels(lImagePlusArray,
                                                     false);
    if (fusedImage != null)
    {
      fusedImage.show();
