...
/**
 * ChangeLightSheetXInstruction allows controlling the scan head to change the X
                                          ...
                                          PropertyIOableInstructionInterface
{
  ...
  private final BoundedVariable<Double> mLightSheetX =
                                                     new BoundedVariable<Double>("Light sheet X",
                                                                                 0.0,
                                                                                 -Double.MAX_VALUE,
                                                                                 Double.MAX_VALUE,
                                                                                 0.01);

  public ChangeLightSheetXInstruction(LightSheetMicroscope pLightSheetMicroscope,
    ...
    for (int cpi = 0; cpi < lState.getNumberOfControlPlanes(); cpi++)
    {
      for (int l = 0; l < lState.getNumberOfLightSheets(); l++)
      {
        lState.getInterpolationTables()
              .set(LightSheetDOF.IX, cpi, l, mLightSheetX.get());
      }
    }
    return true;
  }
  ...
  {
    return new ChangeLightSheetXInstruction(getLightSheetMicroscope(),
  ...
  public String getDescription() {
    return "Change the X-position of an illumination-arm to a given value.";
  }
