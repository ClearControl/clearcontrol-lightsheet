...
public class MeasureImageQualityInstructionPanel extends
                                                 CustomGridPane
{
  public MeasureImageQualityInstructionPanel(MeasureImageQualityInstruction pInstruction)
  {
    StringVariableTextField lTextField =
                                       new StringVariableTextField(pInstruction.getKeyMustContainString()
                                                                               .getName(),
                                                                   pInstruction.getKeyMustContainString());
    add(lTextField.getLabel(), 0, 0);
    add(lTextField.getTextField(), 1, 0);

    HashMap<FocusMeasures.FocusMeasure, Variable<Boolean>> featureMap =
                                                                      pInstruction.getSelectedFeaturesMap();
    int i = 1;
