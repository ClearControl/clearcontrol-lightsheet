 ...
 */
public class ViewStack2DInstruction<T extends StackInterfaceContainer>
                                   extends ViewStackInstructionBase<T>
                                   implements
                                   PropertyIOableInstructionInterface
{

  private BoundedVariable<Integer> mViewerIndexVariable =
                                                        new BoundedVariable<Integer>("Viewer index",
                                                                                     0,
                                                                                     0,
                                                                                     Integer.MAX_VALUE);
   ...
   */
  public ViewStack2DInstruction(int pViewerIndex,
                                Class<T> pTargetStackInterfaceContainerClass,
                                LightSheetMicroscope pLightSheetMicroscope)
  {
    super("Visualisation: View stack '"
          + pTargetStackInterfaceContainerClass.getSimpleName()
          + "' in 2D viewer",
          pTargetStackInterfaceContainerClass,
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    StackInterfaceContainer lContainer =
                                       lDataWarehouse.getOldestContainer(getStackInterfaceContainerClass());
    if (lContainer == null)
    {
      return false;
    }

    Stack2DDisplay lDisplay =
                            (Stack2DDisplay) getLightSheetMicroscope().getDevice(Stack2DDisplay.class,
                                                                                 mViewerIndexVariable.get());
    ...
    lDisplay.getInputStackVariable()
            .set(getImageFromContainer(lContainer));
  ...
  {
    return new ViewStack2DInstruction(mViewerIndexVariable.get(),
                                      getStackInterfaceContainerClass(),
