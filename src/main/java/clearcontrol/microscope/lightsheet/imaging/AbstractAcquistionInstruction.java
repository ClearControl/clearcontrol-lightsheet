 ...
 */
public abstract class AbstractAcquistionInstruction extends
                                                    ...
                                                    LoggingFeature
{
  ...
  protected Variable<String> mChannelName =
                                          new Variable<String>("Dataset name for saving",
                                                               "default");
  ...
  public boolean initialize()
  {
    ...
    mCurrentState =
                  (InterpolatedAcquisitionState) getLightSheetMicroscope().getAcquisitionStateManager()
                                                                          .getCurrentState();
    mTimelapse =
               getLightSheetMicroscope().getDevice(LightSheetTimelapse.class,
                                                   0);
                                     ...
                                     StackInterfaceContainer pContainer)
  {
    ...
    Variable<StackInterface> lStackCopyVariable =
                                                new Variable<StackInterface>("stackcopy",
                                                                             null);
    ElapsedTime.measureForceOutput("Copy stack (" + pKey
                                   + ") for container",
                                   () -> {
                                     lStackCopyVariable.set(lRecycler.getOrWait(1000,
                                                                                TimeUnit.SECONDS,
                                                                                StackRequest.build(pStack.getDimensions())));

                                     // we need to copy the data out of the
                                     // input-buffer from the camera
                                     pStack.getContiguousMemory()
                                           .copyTo(lStackCopyVariable.get()
                                                                     .getContiguousMemory());
                                     lStackCopyVariable.get()
                                                       .setMetaData(pStack.getMetaData()
                                                                          .clone());
                                   });
    if ((lStackCopyVariable.get()
                           .getMetaData()
                           .getTimeStampInNanoseconds()
         - mTimeStampBeforeImaging) < 0)
    {
    ...
    info(pKey + " ("
         ...
         + ") in a container "
         + MetaDataView.getCxLyString(lStackCopyVariable.get()
                                                        .getMetaData()));
                                     ...
                                     double lDetectionZZStart)
  {
    ...
    ((InterpolatedAcquisitionState) lLightsheetMicroscope.getAcquisitionStateManager()
                                                         .getCurrentState()).applyAcquisitionStateAtZ(lQueue,
                                                                                                      lIlluminationZStart);
    for (int l =
               0; l < lLightsheetMicroscope.getNumberOfLightSheets(); l++)
    {
      lQueue.setI(l, false);
      lQueue.setIZ(lIlluminationZStart);
    }
    for (int d =
               0; d < lLightsheetMicroscope.getNumberOfDetectionArms(); d++)
    {
    ...
    lQueue.addCurrentStateToQueue();
    lQueue.addVoxelDimMetaData(lLightsheetMicroscope,
