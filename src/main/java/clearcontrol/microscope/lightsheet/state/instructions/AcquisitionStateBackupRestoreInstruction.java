...
/**
 * This instructions allows saving a copy of the current acquisition state to a
 * list or restoring the latest acquisition state if the list is not empty.
 *
...
{
  boolean mBackup;
  private static ArrayList<AcquisitionStateInterface> mAcquisitionStateList =
                                                                            new ArrayList<>();
   ...
   *
   * @param pBackup:
   *          If true, the instructions puts a new entry in the LIFO list, if
   *          false it will restore the last entry.
   */
  public AcquisitionStateBackupRestoreInstruction(boolean pBackup,
                                                  LightSheetMicroscope pLightSheetMicroscope)
  {
    super("Adaptation: " + (pBackup ? ("Backup") : ("Restore"))
          + " acquisition state",
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    else
    {
      ...
      {
        AcquisitionStateInterface lState =
                                         mAcquisitionStateList.get(mAcquisitionStateList.size()
                                                                   - 1);
        mAcquisitionStateList.remove(mAcquisitionStateList.size()
                                     - 1);
        MicroscopeBase mMicroscope = getLightSheetMicroscope();
        mMicroscope.getAcquisitionStateManager()
                   .setCurrentState(lState);
      }
      else
      {
