...
/**
 * This instructions initializes focus alpha (illumination alpha). It should be
 * used if the focus is really off.
 *
                                                    ...
                                                    PropertyIOableInstructionInterface
{
  ...
  private BoundedVariable<Double> mExposureTimeInSecondsVariable =
                                                                 new BoundedVariable<Double>("exp",
                                                                                             0.05,
                                                                                             0.0000001,
                                                                                             Double.MAX_VALUE,
                                                                                             0.0000001);
  private BoundedVariable<Integer> mImageWidthVariable =
                                                       new BoundedVariable<Integer>("image width",
                                                                                    2048,
                                                                                    16,
                                                                                    2048,
                                                                                    1);
  private BoundedVariable<Integer> mImageHeightVariable =
                                                        new BoundedVariable<Integer>("image height",
                                                                                     2048,
                                                                                     16,
                                                                                     2048,
                                                                                     1);
  private BoundedVariable<Integer> mNumberOfImagesToTakeVariable =
                                                                 new BoundedVariable<Integer>("number of samples",
                                                                                              25,
                                                                                              3,
                                                                                              1000,
                                                                                              1);
  private BoundedVariable<Double> mAlphaStepVariable =
                                                     new BoundedVariable<Double>("alphaRange",
                                                                                 0.5,
                                                                                 0.01,
                                                                                 Double.MAX_VALUE,
                                                                                 0.01);
  ...
  public FocusFinderAlphaByVariationInstruction(int pLightSheetIndex,
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    double alpha = determineBestDeltaAlphaAtZ(lCPPositionZ,
                                              lLightSheetIndex,
                                              lCameraIdx);
    info("determined best alpha " + alpha);

    double lIZ = lAcquisitionState.getInterpolationTables()
  ...
  private double determineBestDeltaAlphaAtZ(double lCPPositionZ,
                                            ...
                                            int lCameraIdx)
  {
    ...
    double lExposureTimeInSeconds =
                                  mExposureTimeInSecondsVariable.get();
    ...
    double lAlphaStart = -lIlluminationAlphaStep
                         * (lNumberOfImagesToTake - 1) / 2;
    ...
    for (int illuminationAlphaIndex =
                                    0; illuminationAlphaIndex < lNumberOfImagesToTake; illuminationAlphaIndex++)
    {
      // #print(" i" + str(lIlluminationZStart))
      // #print(" d" + str(lDetectionZZStart + illuminationZIndex *
      // lDetectionZStep))
      lCurrentState.applyAcquisitionStateAtZ(lQueue,
                                             lIlluminationZStart);
      ...
      double lPositionAlpha = lAlphaStart + illuminationAlphaIndex
                                            * lIlluminationAlphaStep;
    ...
    StackInterface lStack =
                          getLightSheetMicroscope().getCameraStackVariable(lDetectionArmIndex)
                                                   .get();
    ...
    {
      info("arr "
           + Arrays.toString(lQualityPerSliceMeasurementsArray));
    ...
    double lArgMaxQualityDeltaZ =
                                lSmartArgMaxFinder.argmax(lXAxis,
                                                          lQualityPerSliceMeasurementsArray);

    if (debug)
    {
      info("bef "
           + lQualityPerSliceMeasurementsArray[lNumberOfImagesToTake
                                               / 2]);
                                   ...
                                   double lDetectionZZStart)
  {
    ((InterpolatedAcquisitionState) lLightsheetMicroscope.getAcquisitionStateManager()
                                                         .getCurrentState()).applyAcquisitionStateAtZ(lQueue,
                                                                                                      lIlluminationZStart);

    for (int i =
               0; i < lLightsheetMicroscope.getNumberOfLightSheets(); i++)
    {
      lQueue.setI(i, false);
  ...
  {
    FocusFinderAlphaByVariationInstruction copied =
                                                  new FocusFinderAlphaByVariationInstruction(mLightSheetIndex,
                                                                                             mDetectionArmIndex,
                                                                                             mControlPlaneIndex,
