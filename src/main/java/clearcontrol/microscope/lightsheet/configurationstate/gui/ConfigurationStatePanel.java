...
public class ConfigurationStatePanel extends CustomGridPane
{
  ...
  public ConfigurationStatePanel(ArrayList lObjectList,
                                 int pNumberOfLightSheets)
  {
    ...
    for (int x = 0; x < lObjectList.size(); x++)
    {
      ...
      if (lObject instanceof HasConfigurationState
          && lObject instanceof ReadOnlyNameableInterface)
      {
        Label lLabel =
                     new Label(((ReadOnlyNameableInterface) lObject).getName());
        add(lLabel, posX + 1, 0);
        posX++;
      }
    }
    ...
    for (int x = 0; x < lObjectList.size(); x++)
    {
      ...
      if (lObject instanceof HasConfigurationState
          && lObject instanceof ReadOnlyNameableInterface)
      {
        HasConfigurationState lHasConfigurationState =
                                                     (HasConfigurationState) lObject;
        if (lHasConfigurationState instanceof HasConfigurationStatePerLightSheet)
        {
          HasConfigurationStatePerLightSheet lHasConfigurationStatePerLightSheet =
          ...
          for (int y = 0; y < pNumberOfLightSheets; y++)
          {
            ...
            lHasConfigurationStatePerLightSheet.addConfigurationStateChangeListener(new ConfigurationStatePerLightSheetChangeListener()
            {
              ...
              @Override
              public void configurationStateOfLightSheetChanged(HasConfigurationStatePerLightSheet pHasConfigurationStatePerLightSheet,
                                                                int pLightSheetIndex)
              {
                if (pLightSheetIndex == lLightSheetIndex)
                {
                  ConfigurationState lConfigurationState =
                                                         pHasConfigurationStatePerLightSheet.getConfigurationState(pLightSheetIndex);
        ...
        else
        {
          ...
          lHasConfigurationState.addConfigurationStateChangeListener(new ConfigurationStateChangeListener()
          {
            ...
            public void configurationStateChanged(HasConfigurationState pHasConfigurationState)
            {
              ...
              lVariableLabel.setStyle(" -fx-padding: 2 2 2 2; -fx-border-color:white; -fx-text-fill:white; -fx-background-color: "
                                      + lConfigurationState.getColor()
                                                           .toLowerCase()
                                      + ";");
            }
