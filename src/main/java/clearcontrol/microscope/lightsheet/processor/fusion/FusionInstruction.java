 ...
 * operation. Depending on how the images were acquired, they might have to be
 * passed differently to the FastFuse engine. Details are available in derived
 * classes.
 *
        ...
        AutoRecyclerInstructionInterface
{
  ...
  protected StackInterface fuseStacks(StackInterfaceContainer pContainer,
                                      String[] pImageKeys)
  {
    if (pContainer == null)
    {
    ...
    final LightSheetFastFusionProcessor lProcessor =
                                                   getLightSheetMicroscope().getDevice(LightSheetFastFusionProcessor.class,
                                                                                       0);
    ...
    ElapsedTime.measure("Handle container (" + pContainer
                        + ") and fuse",
                        () -> {
                          synchronized (mLock)
                          {
                            info("available keys in container: "
                                 + pContainer.keySet());
                            info("needed keys in container: "
                                 + Arrays.toString(pImageKeys));
                            for (String key : pImageKeys)
                            {
                              StackInterface lResultingStack =
                                                             pContainer.get(key);
                              StackInterface lStackInterface =
                                                             lProcessor.process(lResultingStack,
                                                                                lRecycler);
                            ...
                            if (mFusedStack == null)
                            {
                              lProcessor.getEngine()
                                        .executeAllTasks();
                              warning("Finished, but there are just "
                                      + lProcessor.getEngine()
                                                  .getAvailableImagesSlotKeys());
                            }
                          }
                        });
  ...
  protected void resetEngine(int pNumberOfLightSheets,
                             int pNumberOfDetectionArms)
  {
    ...
    lProcessor.getEngine().setup(pNumberOfLightSheets,
  ...
  protected void storeFusedContainer(StackInterface lFusedStack)
  {
    long lTimePoint =
                    lFusedStack.getMetaData()
                               .getValue(MetaDataOrdinals.TimePoint);
    DataWarehouse lDataWarehouse =
                                 getLightSheetMicroscope().getDataWarehouse();
    FusedImageDataContainer lFusedContainer =
